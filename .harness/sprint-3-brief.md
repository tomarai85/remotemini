...
`nextHistoryLimit` のケースは **5件**(`test/view.test.mjs:373-378`)。同じく全部移す事:

| # | JS の assertion | 期待 |
|---|---|---|
| 1 | `nextHistoryLimit(0)` | **150**(「まだ0件でも先へ進む」。★これが `||` と `??` の割れ目 = §2-b-4 の訂正6-1) |
| 2 | `nextHistoryLimit(50)` | 150 |
| 3 | `nextHistoryLimit(120) > 120` | 現在値より必ず大きい |
| 4 | `nextHistoryLimit(480)` | 500(上限で止まる) |
| 5 | `nextHistoryLimit(500)` | 500(自分自身を返す = §3-b の頭打ち判定の根拠) |

★**この表は訂正6-1 と同時に足した(2026-08-05 11:15)**。初版は §2-a に
「JS の検査ケースを1件残らず Swift へ移す」と**規則だけ**書き、`mergeHistory` の 6 件は表にしたのに
`nextHistoryLimit` は表にしなかった。規則の適用範囲は両方なのに、**それを数える道具が片方にしか
無かった**ので、移し忘れても誰も気付かない。今夜 `staged-controls-gate.sh` / `run-controls.sh` /
`pre-commit` で4回続けて踏んだのと**同じ形**(DESIGN §2.18-10)—— 検査の本体ではなく
**ブリーフの中で**同じ事が起きていた。分母は書いた時にしか書けない。
...
4. `nextHistoryLimit(current) = Math.min(500, (current || 50) + 100)`。**500 で頭打ち**である事が
   §3-b の裁定の根拠なので、`min` を落とさない。
   —— **★訂正6-1(2026-08-05 11:15)。このブリーフの初版はここを `(current ?? 50)` と書いていた。
   出所は `||` であって `??` ではない**(`rc-backend/src/view.mjs:332`、実測)。Swift の `??` は
   `nil` しか拾わないので `0` が素通りし、**`nextHistoryLimit(0)` が JS の 150 に対し 100 を返す**。
   JS 側にはこの入力を名指しした検査が在る(`test/view.test.mjs:374`、理由付きで
   「まだ0件でも先へ進む」)。**写し取りの正しい形**:

   ```swift
   static func nextHistoryLimit(_ current: Int?) -> Int {
       // JS の `||` は falsy を拾うので `0` も fallback へ落ちる。Swift の `??` は
       // `nil` しか拾わないから、`0` を `nil` と同じ側へ寄せないと 150 が 100 になる。
       let base = (current == 0 ? nil : current) ?? 50
       return min(500, base + 100)
   }
   ```

   今の実装(`?? 50`)でも**画面は壊れない** —— `currentLimit` は非 optional の `Int` で
   `initialLimit = 50` から始まり、以後は `nextHistoryLimit` の戻り値(100 以上)しか入らないので、
   **`0` はアプリ内のどの経路でも到達しない**(実測: 宣言1 + 代入2箇所を全部追った)。
   それでも直す理由は2つ。① C群の担保は「**サーバ側の検査が電話側の証拠になる**」事であって、
   JS の検査が名指しした入力で答えが割れているなら、その担保は既に切れている。
   ② より悪いのは**注釈が出所を偽って引用している**事 —— `MergeHistory.swift:55` と
   `NextHistoryLimitTests.swift:4` の両方が JS を `(current ?? 50)` と引いている。
   後から突き合わせる人が、一致していない物を注釈だけ見て「一致」と読む。
