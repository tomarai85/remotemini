...
- ★**この対照の所要は単一の値ではない**(xcodebuild 14 回 = 基準1 + 変異13)。
  ~~9 分前後・11 回~~ は古い版。2026-08-06 に 2 回とも門から実測:
  **ios/Sources を含む commit = 1651s / `.harness/*.sh` だけの commit = 247s**(§6-6)。
  同じ日に「11 分 → 27 分」と直した直後に 4.1 分が出た = **1 標本で「前後」と書いていた**。
  9 分を信じて `git commit` の timeout を短く置いた結果 SIGKILL が変異を取り残したので、
  運用規則は数字に依存しない形にした: **この門を待つ commit は背景で回す**(前景 timeout 禁止)。
...
## 6-6. 本文を守る筈の対照が、本文を汚す側に回っていた(2026-08-06)

`dod-sprint-6-controls.sh` は Swift 4 file を書き換えて戻す。その復元が
**走行を SIGKILL された時だけ走らない**。机上の心配ではなく、この晩に実際に起きた。

### 起きた事

`git commit` を Bash の timeout 2m → 9m20s で 2 回殺した。2 回目の SIGKILL の時、
`ReachabilityMeter.swift` の変異(`consecutiveFailures = 0` を減衰式へ)が
**未 staged のまま作業木に残った**。

未 staged (` M`) は見た目が無害な一方、`git add -A` は拾う。つまり
**対照が commit を汚す経路**が出来ていた。気付いたのは `git status` を読んだからで、
検査が教えてくれたのではない。

### 何故 timeout を 2m と 9m20s に置いたか —— 見積りが道具の設定値になる

対照の冒頭が自分で「所要 11 分前後」と書いていた。実測は **1651s(27.5 分)**。
私はその 11 分を信じて timeout を決めた。

**待ち時間の見積りは、待つ側の道具の設定値になる。ずれた見積りは中断を作る。**

### ★その訂正自体が、同じ間違いだった

冒頭を「27 分前後」へ直した。**その直後の走行が 247s(4.1 分)** —— 6.7 倍のずれ。
一つの誤った単一値を、別の単一値に置き換えていただけだった。

| commit の中身 | 実測 | なぜ |
|---|---|---|
| `ios/Sources` を含む(`0e8a333`) | **1651s** | DerivedData が無効化され基準ビルドが素から。13 変異も巻き添え |
| `.harness/*.sh` だけ(`864ebf6`) | **247s** | 増分が効き、変異は 1 file ずつ |

**1 標本で「前後」と書くな。** 費用は「この commit が `ios/Sources` を触るか」で決まるので、
冒頭にはそう書き直した。

そして運用側の規則を**数字に依存しない形**へ変えた ——
「27 分だから timeout を 30 分にする」ではなく
**「この門を待つ commit は前景で timeout を掛けず、背景で回して終了を待つ」**。
見積りを当てられなくても壊れない形にしないと、次の 6.7 倍でまた同じ中断を作る。

### 最初の仮説は probe で否定された

「`trap cleanup EXIT` は SIGTERM を取り落とすので `INT TERM HUP` を足せば直る」と考え、
足す直前に測った。対象の構造(複製→変異→走行→復元)を縮めた probe:

| trap | signal | 復元 |
|---|---|---|
| `EXIT` | SIGTERM | **戻る** |
| `EXIT` | SIGKILL | 戻らない |
| `EXIT INT TERM HUP` | SIGKILL | **やはり戻らない** |

仮説は外れ。SIGTERM は元から拾えていて、拾えないのは SIGKILL だけ。
SIGKILL は trap を通らないので、**signal を足す修正は測って無効**。
効くのは復旧側 —— 「次に起きた時に前回の残骸を拾う」しかない。

冒頭にあった「trap EXIT で復元。中断されても作業木は元に戻る」も、これで偽と確定したので
実測表ごと本文へ書き換えた。**対照が自分について嘘を書いている**のが一番たちが悪い。

### 直し方 —— 固定 path の目印 + 次回起動時の復旧

複製を全部取った後に `${TMPDIR}/rc-dod-sprint6-inflight.tsv` へ「対象→複製」を書く。
次の走行は、走り出す前にその目印を読んで、中身が違う file を複製から戻す。

順序に制約が3つ在り、どれも破ると嘘か取りこぼしを作る:

| 制約 | 破ると何が起きるか |
|---|---|
| 復旧は複製 loop より**前** | 変異したバイトを「走る前の中身」として複製し、**復元の基準点そのものが汚れる** |
| 目印は全複製が取れた**後**に書く | 複製に失敗して exit 2 した回が「戻せる複製が在る」と嘘の申告を残す |
| 目印の削除は復元の**後** | 消した直後に殺されると「戻っていないのに手掛かりも無い」状態が作れる |

複製が消えていて戻せない時は、緑でも赤でもなく **exit 2(測定不能)**。清潔だと言い張らない。

### 対照 19 本(`.harness/dod-sprint-6-recovery-controls.sh`)

復旧区間を**錨で切り出して**回す。写しは作らない —— 写した瞬間、測っている物が写しになり、
的が変わっても緑のままになる。錨が一意でなければ exit 2。xcodebuild も Swift も要らず数秒。

歯は陰性対照2つで測った:

| 陰性対照 | 潰した物 | 期待どおり赤くなった検査 |
|---|---|---|
| A | 戻す `cp` を `:` へ | 「複製から戻る」 |
| B | 目印の削除を `:` へ | 「戻したら目印を消す」 |

どちらも変異が空振りしたら exit 2 で止まる(`cmp -s` で確認してから使う)。
砂場(repo の外の偽の的)で 19/19 → 本物の的で 19/19、33 行を切り出し。

### 繋ぎも実測した —— 単体で緑でも「殺された次に拾うか」は別の話

切り出した区間が緑なのは区間の話でしかないので、本物で通した:
`ReachabilityMeter.swift` に変異を植え、目印を書き、**本物の対照を起動**して、
`復旧: 前回の走行が殺されて残っていた変異を戻した: ios/Sources/Core/ReachabilityMeter.swift`
が出て本文が戻る所まで観測した。

★この時、私は最後の判定を1つ**読み違えた**。「目印が残っている = 失敗」と書いたが、
殺した走行は既に「復旧 → 複製取得 → **新しい目印を書く**」まで進んでいたので、
残っていたのは前回の残骸ではなく**今回ぶんの正しい目印**だった。中身を開いたら 4 本とも
複製と一致していて、変異は植わる前だった。設計どおりの挙動を、検査の側が失敗と読んだ形。

### 付随して判った門の挙動(見込みが外れた)

この commit は `.harness/*.sh` しか staged していないので 27 分の対照は引かれない、と
見込んでいた。`staged-controls-gate.sh` を読んだら外れ —— **staged された対照は
(a) の分岐で自分自身が選ばれる**。変えたのだから走るのが正しい。見込みで済ませず
読んだので、また timeout を短く置く事にはならなかった。

## 6-7. 門の入口の「手書きの絞り込み」が、どれだけ穴なのかを初めて数えた(2026-08-06)

`pre-commit-gates.sh` の早期 `exit 0` は手書きの regex 1 本で、
`staged-controls-gate.sh` の `SCAN_SPECS` と**手で同期する 2 本目の一覧**。
注釈自身が「これが 5 回の再発の真因」と名指ししながら、
**穴の大きさは一度も数えていなかった**(「見えている穴」とだけ書いてあった)。

数え方: 全対照の `# controls-for:` 宣言を基点込みで repo 根からの形に直し、
**その宣言先だけを staged した commit が早期 `exit 0` を通るか**を 1 本ずつ当てる。

| 測った物 | 数 |
|---|---|
| 宣言の総数(repo 内) | 75 |
| repo 外(`external:` = staged から原理的に選べない) | 1 |
| 早期 `exit 0` を**通る**(= 門に届く) | 74 |
| ★**門ゼロ**(= 宣言されているのに、その file だけの commit では一度も走らない) | **1** |

門ゼロの 1 本は `rc-backend/package.json`(`copied-tree-controls.sh` が宣言)。
package.json だけの commit は、その対照だけでなく
**`npm test` の門(commit-suite-gate)ごと飛ぶ** —— `test` script を書き換える commit が
一番検査されるべき場面で、下流の門が 1 つも走らない形になっている。

★ただし**履歴上の発生は 0 件**。package.json に触れた commit は 2 件在り、
どちらも他の file を一緒に含むので門は走っていた。
(最初に「2 件が該当」と書いたのは私の判定式の誤り —— 空出力と多 file を
取り違える書き方をしていた。直接当て直した答えが 0 件。)

### 直し方を「regex に 1 行足す」にしない理由

`|^rc-backend/package\.json$` を足せば 1 行で閉じる。だがそれは
**手で同期する一覧を 6 個目の項目に増やす**手で、5 回連続の再発を作ったのと同じ形。
根治は「絞り込みが対照の一覧を推測しない」事 ——
`staged-controls-gate.sh` に**選ぶだけで走らせない mode** を足し、
入口はそれに聞く。一覧は 1 本のまま、届く範囲は下流の能力に自動で追随する。

**この節では実装しない**。理由は測って決めた事ではなく段取り: この時点で
`staged-controls-gate.sh` は走行中の commit が**読みながら実行中**で、
bash は script を逐次読むので、走っている物を書き換えると実行が壊れる。
commit が着地してから触る。

### 実装(同じ晩、commit が着地した後)

**足した物は 3 行**。`staged-controls-gate.sh` の選択が済んだ直後に:
...
置き場所が仕様そのものなので、3 つとも理由が在る:

| 位置 | 何を跨がない為か |
|---|---|
| `undecl` の停止より**前** | 聞いただけで commit が落ちない(口は裁かない) |
| edith 側の除外より**前** | 落とした結果が空でも、入口は門へ進む(安全側に外れる) |
| `set +f` より**前** | `$sel` を分割する時に path が glob 展開されない |

呼ぶ側(`pre-commit-gates.sh`)は regex を**据え置いたまま**、外れた時に即断せず
「回る物は在るか」だけ聞く。書類だけの commit に足す費用は**実測 0.1 秒未満**
(本番の走行は 20〜1651 秒なので、聞く/走らせるの差は 3 桁)。

### ★足してから気付いた: 隣に `--list` が既に在った

同じ file の下の方に `--list`(選び方だけ見せて回さない)が既に在り、
**足す前に見つけるべきだった**。「一覧を 2 本持つな」と書いている最中に、
似た口を 2 つ目に増やしかけている。

突き合わせた結果は「消す」ではなく「**違いを本文に書いて 3 つ目を作らせない**」:

| | `--would-select`(機械向け) | `--list`(人向け) |
|---|---|---|
| 選択の道 | 同じ `$sel`(別実装なし) | 同じ `$sel` |
| `undecl` | 跨ぐ(裁かない) | `exit 1` で止まる |
| edith 側 | 落とす**前** | 落とした後 |
| 選択が空の時 | **完全に無言** | `触れた対照は無い` と喋る |

入口がこの口でなければならない理由は 2 つとも「喋る/落とす」に在る:
① `--list` は空でも喋るので `[ -z "$(…)" ]` が常に非空 = 絞り込みが素通しに化ける。
② `--list` は edith 側を落とすので、落ちて空になった commit が
`commit-suite-gate`(npm test)ごと飛ぶ —— 塞ぎたかった穴と同じ形に戻る。
結果、この口の答えは `--list` の**上位集合**になる。わざとの差なので latent にせず
assertion にした(S47 / S47b)。

### 自分の変更が、既存の対照を 5 本倒した(直す前に数えた)

`pre-commit-gates-controls.sh` は「門の呼び出し」を `bash "$ROOT/…/x.sh"` の形で
grep して一覧にしている。新しく足した**探りの一行も同じ形**なので、
`staged-controls-gate` が一覧に 2 回入り、門の本数が 5→6 に、
`sed -n '2p'` で選ぶ「赤にする門」の位置も 1 つずれた。

12 本中 5 本が赤。**5 本とも根が 1 つ**(門の本数 / 書類だけの 3 通 / 赤の後ろ)。
直しは 2 箇所:抽出から `--would-select` の行を除く(探りは門の呼び出しではない)、
偽の門の stub が `--would-select` に**記録せずに**答える様にする。

### 結果

| 対照 | 前 | 後 |
|---|---|---|
| `pre-commit-gates-controls.sh` | 12 本中 5 赤 | **15/0**(新規 3 本 = 10 / 10b / 10c、10c は陰性対照) |
| `staged-controls-gate-controls.sh` | 44/44 | **55/0**(新規 11 本 = S42〜S48) |

新規 11 本が測っている契約は 4 点 —— ①本番と同じ物を選ぶ(`--list` と一致)
②選んだ物を**回さない** ③宣言の無い対照でも裁かない ④選ぶ物が無ければ**無言**。
...
disk 上の対照 **57 本**(訂正。下記「★訂正2件」参照)、どの一覧にも載っていない物 **8 本**:
...
#### ★訂正2件(この段落を書いた直後に測り直して判った)

**① 「56 本」は古い値だった。正は 57 本。**
数えたのが `ios/tools/conversation-ui-control.sh` を作業木に昇格させる**前**で、書く時点で
測り直していない。分母を落とした主張を1本出しかけた —— 差は1本で結論は変わらないが、
**測った時刻と書いた時刻が違う数字を、測り直さずに本文へ置いた**のが問題の方。
再測(2026-08-06): `disk=57 / unreg=8`。

**② 「上の4本は今週 commit した物」も誤りだったので、その一文ごと削った。**
`git log --diff-filter=A` で1本ずつ入った時刻を引いたら、**8本とも 2026-08-06 の同じ日**に
入っていた:
...
そして `run-controls.sh` は **2026-08-05 21:41 の `a3d6b58`(前回「登録漏れ6本」を
まとめて直した commit)から今日まで一度も触られていない**。
つまりこれは「今週ぶん溜まった」話ではなく、**前回の是正の翌日に、同じ形が丸一日ぶん
再発した**話。「今週」と書いていたら、再発の速さ(1日)が記録から消えていた。

#### だから記録ではなく門にした —— `tools/controls-registration-gate.sh`

前回この形を直した時に得た教訓は「緑を出した instrument が**何を測っているか言える事**」で、
それは**言える様になる**事しか要求していない。言えても、次に書く対照が登録される訳ではない。
登録を守っていたのは私の記憶だけで、記憶に懸けた守りは1日で戻った。

門の要求は1つだけ: **対照 file を新設(A)または改名(R)した commit は、同じ commit で
`rc-backend/tools/run-controls.sh` も staged にする事**。どの一覧(LOCAL / EDITH / EXCLUDED)へ
入れたかは問わない —— EXCLUDED の理由必須はあちらの掟が既に見ている。此処が見るのは
「触ったか」だけ。既存の対照を**変更しただけ**では止めない(毎回止まる門は外される)。

- ★**手書きの dir 一覧を持たない**。名前(`*-control.sh` / `*-controls.sh`)だけで判る形にした。
  この repo が5回踏んだ再発の真因は「手で同期する一覧が2本」なので、2本目を作らない。
  走査 dir の外に対照を置く事自体が別の穴なので、**どこに置いても**止める方が強い。
  代償は、対照でない道具に同じ名前を付けた時に1度だけ誤って止まる事(現在該当する3本は
  既に追跡済なので A でも R でもなく当たらない)。その時は降ろす口に理由を書く。
- 呼ぶ位置は `pre-commit-gates.sh` の**絞り込みより前**、`doc-linerefs-gate.sh` の隣に無条件。
  条件をあちら側に書くと手で同期する一覧がまた1本増える —— それが5回続いた再発の真因。
- 終了コードは 0/1/**2**。git として読めない時を緑に丸めない。
- 降ろす口 `RC_CTLREG_OK='理由'` は理由を出力に残す(空では降りない)。

対照 `test/controls-registration-gate-controls.sh` は偽の repo を `git init` して門を撃つ。
9本:新設だけ=赤で file を名指し / 一覧も触った=緑 / 変更だけ=素通し / **走査 dir の外でも赤** /
改名でも赤 / 対照が無い commit は**無言**の緑 / 降ろす口は理由を残す / git でなければ exit 2 /
★**陰性対照** = 門の `exit 1` を潰した写しでは「新設だけ」が緑に変わる(赤が本物の歯から
来ている事の証明)。実測 `PASS 9 / FAIL 0`、3 秒。
...
### 2026-08-06 22:15 定期掃きの初回が、commit の門に見えない欠陥を捕まえた

門を積んだ直後の全掃き(`tools/run-controls.sh`、56 本・12 分 45 秒)の結果:

```
RUN-CONTROLS: green=52 red=3 未測定=1  (対象 56本、edith専用2本は除外)
  赤: no-linerefs-controls.sh mutation-freeze-controls.sh copied-tree-controls.sh
  未測定: mutation-verdict-controls.sh
```

3本の赤は**全部同じ1つの原因**だった: **rc-backend だけを写した木で、単体スイートが緑でない**。

#### なぜそれが最悪の場所か

`test/mutation-controls.py` は rc-backend **だけ**を temp へ写し、その写しで `npm test` を
回す。走行の最初に立てる対照1(無変異の木は緑)がそれで、落ちれば台本は `die()` する
= **変異が1件も回らない**。単体スイートは変異走行の判定器なので、「写しで回る」は
スイートの機能要件である。つまりこの3時間、**この repo の判定器は丸ごと死んでいた**。

#### 何時から死んでいたか(git の object から写しを起こして再現)

作業木ではなく `git archive <rev> rc-backend | tar -x` で起こした(掃きが同時に木を
写している最中だったので、作業木から測ると掃きの細工を拾う)。

| rev | tests | fail | 中身 |
|---|---|---|---|
| `135fdca`(`0326609` の親) | 681 | **0** | 健全 |
| `0326609` | 687 | **6** | 新設2本が木の外を無条件に読む |
| `4f3b474`(掃き時点の HEAD) | 687 | **7** | 上に私の裸の引用が1件 |

- 6本 = `test/fixture-reader-declared.test.mjs`(ios/Sources を走査)と
  `test/mutation-recovery-copy.test.mjs`(ios と .harness の対照2本を読む)。
  どちらも 19 時台の `0326609` で私が足した物で、**手元では 687/687 緑**。
- 7本目 = `tools/run-controls.sh` の注釈で ios の対照を**裸**(`/` 無し)で2回引いた件。
  裸の引用は隣の木でも解決するので、完全な木では緑・写しの中だけで赤になる。

#### 門が構造的に見られない形である

commit の門(`tools/commit-suite-gate.sh`)が回すのは**完全な木**の `npm test` で、そこには
ios が在るので全部緑になる。写しの形を踏むのは定期掃きの `mutation-freeze-controls.sh` と
`copied-tree-controls.sh` だけ。だから門は3回とも素通しし、12 分の掃きが初回で捕まえた。
**緑がどの木で出た緑なのかを言わない限り、緑は緑ではない。**

#### 直し方 —— 2値に丸めない

木の外の対象は3通りに分かれる。ここを赤/緑の2値に丸めていたのが欠陥の本体:

| 状態 | 正しい扱い |
|---|---|
| 対象が在る | 測る |
| 対象が無い + 親も欠け | **測っていない**と名乗って飛ばす(赤にしない) |
| 対象が無い + 親は健在 | 赤(改名・削除は捕まえるべき) |

分岐は `test/subtree.mjs` に1つだけ置いた(`requireOutside(rels)` → `{skip, missing, measured}`)。
飛ばしは node:test の `{ skip: "理由" }` なので TAP に理由ごと出て `# skipped` が増える
—— 黙って消えない。親の健在は `DESIGN.md` の実在だけで見る(`no-linerefs.test.mjs` と同じ目印)。

★**逃げ道の錨**を各 file に1本ずつ入れた: 「この木では『部分木だから測らない』を通っていない」。
`REPO_INTACT` だけを見るので GATE の壊れ方と独立で、常に飛ぶ側へ壊れたら完全な木で赤くなる。
対照 `test/subtree.test.mjs`(6本)が3分岐を砂場の木で実際に踏む。

#### 門の側も1つ塞いだ —— `no-linerefs.test.mjs` の ②-b

裸の引用の向きは**非対称**である。単独で写される木(`rc-backend`)の注釈が隣の木の名前を
裸で引くのは事故、逆(ios → backend)は正常 —— `ios/` だけを単独で写す経路がどこにも無いから。
その非対称を規則にして、**完全な木の側で**捕まえられる様にした(陰性対照つき、4主張)。
この規則が無い間、この形は 12 分の掃きでしか見えなかった。

★2026-08-05 の同じ header に「該当 0件なので規則を増やしていない」と書いてある。
**居ないのは書いていないからで、書けば翌日に現れる**。0 件は規則を省く理由にならない。

#### 実測(直した後)

| 何を | 結果 |
|---|---|
| 完全な木の単体 | **697 / 697 緑・skipped 0**(= 完全な木では逃げ道を通っていない) |
| rc-backend だけの写し | **697 件 / fail 0 / skipped 15**、飛ばした 15 本は全部理由つき |
| `test/no-linerefs.test.mjs` 単体 | 13 / 13 緑(②-b と陰性対照を含む) |
| `test/subtree.test.mjs` 単体 | 6 / 6 緑(3分岐を砂場で両方向に踏む) |
| `test/no-linerefs-controls.sh` | PASS 11 / FAIL 0 |
| `test/copied-tree-controls.sh` | pass=4 fail=0(緑の文言に `skipped=15` が載る) |
| `test/mutation-freeze-controls.sh` | pass=6 fail=0(= 変異走行が現に起動する事の直接証拠) |
| `test/mutation-verdict-controls.sh` | **pass=27 fail=0** |

掃きの4本目 `mutation-verdict-controls.sh` は当時「未測定(pass=10 fail=3)」で、行に
「条件が揃ってから回し直す事」と書いた。直した後に回して 27/0 —— **あの未測定は
独立した欠陥ではなく、死んでいた判定器の下流**だった。掃きが挙げた 4 本(赤3 + 未測定1)は
全部この1つの根に繋がっており、逆に言えば**根を1つ直すまで、どれを個別に見ても直らない**。

#### この件で足した装置(4つ、どれも「一覧を手で揃える」ではない)

| 装置 | 何を機械が見るか |
|---|---|
| `test/subtree.mjs` | 3状態の分岐を1箇所に集約(在る / 写しで無い / 親健在で無い) |
| `test/subtree.test.mjs` | その分岐を砂場で**両方向**に踏む(常に飛ぶ側・常に赤い側の双方が壊れ方) |
| 各 file の逃げ道の錨 | 完全な木で「飛んでいない」事を主張。`REPO_INTACT` は `DESIGN.md` だけを見るので GATE の壊れ方と独立 |
| `no-linerefs.test.mjs` ②-b | 単独で写される木(`rc-backend`)からの**裸の**引用が、その木の中で解決するか |

### 2026-08-06 23:0x 8/02 の教訓が、4本の計器のうち1本にしか適用されていなかった

掃きの後始末として `tools/live-http-check.mjs` の**対照が1本も無い**事に手を付けた
(終了コード `prepAbort?2:failed?1:limitedReply?3:0` は実機でしか走らない台本の結論なのに、
手元で撃つ道が無かった)。そこで4本を横に並べて数えたら、別の物が出た。

| 台本 | 上限を知る | exit 3 |
|---|---|---|
| `tools/live-inject-check.mjs` | あり | あり |
| `tools/live-fork-check.mjs` | あり(自前の写し) | あり |
| `tools/live-http-check.mjs` | あり | あり |
| `tools/live-choice-check.mjs` | **無し** | **無し**(`failed ? 1 : 0`) |

8/02 の incident は「同じ機械を同じ時刻に測った2本が逆の事を言った」で、直し方は
exit 3(= 運ぶ層は通ったが相手が答えていない)を足す事だった。**足したのは1本だけ**。
更に choice 側の注釈は「上限や信頼確認の可能性」と2つの状態を束ねていて、これは
inject 側が 8/02 に解いた束ねと**同じ形**。教訓を1本に当てて他へ運ばない型そのもの。

直した形:
- `tools/exit-codes.mjs` 新設 = 意味(0/1/2/3)と順序(2 > 1 > 3 > 0)の正本。
  順序の理由も此処に書いた —— 2 が最優先(何も測れていない回に測った顔をさせない)、
  1 が 3 より先(上限は「送れない」ではないので運ぶ層の赤は上限で説明が付かない)。
- `live-choice-check.mjs` = 束ねを解いて上限は 3、信頼確認等は 1。`exitCodeFor` に委ねる。
- `live-http-check.mjs` = 三項演算子を `exitCodeFor` に置換(結論の1行が検査できる様に)。
- `test/live-exit-codes.test.mjs` 新設 21本 = 8通り全部 + 4本の合意 + 包含関係。
  ★一覧は手書きだが、`tools/live-*.mjs` を disk から数えて突き合わせるので**5本目は必ず止まる**。

#### 陰性対照で、自分の検査の穴が1つ出た(実測)

| 細工 | 赤 |
|---|---|
| 順序を 1 と 3 で入替 | 2 |
| choice が上限の判定を**呼ばない**(枝を潰す。import は残る) | **0 → 直して 1** |
| 委ね方の旗の名前を入替(`failed` と `limitedReply`) | 1 |
| 写しの判定を狭めて正本を包含しなくする | 1 |
| 台本が1本増える(一覧に無い) | 1 |
| 一覧に足すだけで済ます(3 を持たせない) | 3 |
| 全部戻す | 0 |

2行目が最初 **0** だった。`limitNoticeIn` の文字列は import 行に残るので、素の文字列検査は
緑のまま通った。**import は use ではない** —— 名前の実在を意味と取り違える、この repo が
何度も踏んでいる型。呼び出しの形(`limitNoticeIn(` に引数)を見る様に直して 1 になった。
★もう1つは私の測り方の誤り: 5本目の細工の前に前の細工を戻していなかったので赤が 2 に見えた。
**細工を1つずつ戻してから次を撃つ**(混ざった赤は本数が意味を持たない)。

#### 測っていないと明示する事

`live-fork-check.mjs` は上限の判定を**自前で持っている**(正本は画面に当てるので狭く、
fork 側は自分で固定した prompt の JSON にしか当てないので広く取れる)。両者どちらの綴りが
Anthropic の実際の文面と合っているかは**測っていない** —— 観測は 8/02 の1件のみ。
検査が固定したのは真偽ではなく**包含関係**(正本が上限と読む文面を写しが落とさない)。

実測: 完全な木 **718 / 718 緑・skipped 0** / 写し **718 件 fail 0 skipped 15**(据え置き=
この検査は木の外を要らないので写しでも回る)/ `test/copied-tree-controls.sh` pass=4 fail=0。

### 2026-08-06 23:5x その census 自身が、自分の class を数え落としていた

一つ前の節で「`tools/live-*.mjs` を disk から数えて突き合わせるので**5本目は必ず止まる**」と
書いた。書いた1時間後に、その突き合わせが数えていない集合を見つけた ——
`ios/tools/live-*.sh` の**2本**。教訓を N 箇所のうち一部にしか運ばない型が、
**其れを直す為に建てた道具の中**に出た。

出方はこう: `ios/tools/live-interrupt-check.sh` の「60 秒回しても生成中を1度も観測できなかった」
枝を読みに行った(**一度も走った事が無い枝**を探していた)。読んだら、其れは
**上限の時に必ず通る枝**でもあった —— そして 2 (測れていない) を返していた。
`.mjs` 側が同じ日に足した 3 (運ぶ層は通ったが相手が答えていない) を、shell の2本は持っていない。

| 計器 | 言語 | 上限を知っていたか |
|---|---|---|
| `tools/live-inject-check.mjs` | mjs | あり(8/02) |
| `tools/live-fork-check.mjs` / `live-http-check.mjs` / `live-choice-check.mjs` | mjs | あり(23:0x で揃えた) |
| `../ios/tools/live-send-check.sh` | **sh** | **無し** |
| `../ios/tools/live-interrupt-check.sh` | **sh** | **無し** |

#### 訊く口が無かったので、答える側から作った

`classifyScreen` は既に `limited` を返し、`server.mjs` の応答の土台も `view.mjs` も其れを
`state` とは独立に表示している。**shell から訊く口だけが無かった**。
`tools/disposable-session.mjs` に `limited <id>` を足し、`busy` と共通の前段
(`classifyFor` = 登録簿を読んで pane を撮って分類する)を1つに畳んだ。
畳んだのは「登録簿の読み方が2箇所に在ると片方だけ直る日が来る」= この repo が何度も踏んだ型。

#### 判定を切り出して、真理値表を当てた(21 本)

`live-send-check.sh` の判定は edith と実機の会話が要る = **一度も走った事が無い**まま
書かれていた。`send_verdict()` として切り出し、観測値を引数で受ける `--verdict` を生やした
(`conversation-ui-control.sh` と同じ手)。対照 = `ios/tools/live-send-check-control.sh`、実測1秒未満。

#### ★終了コードだけを見る対照には、穴が在った(陰性対照④)

| 細工 | 赤 |
|---|---|
| ① 細工なし | 0 |
| ② 上限なら常に 3(順序 1 > 3 を壊す) | 10 |
| ③ 訊けなかった("")を上限扱いにする | 1 |
| ④ 上限で落ちた足を `--(測っていない)` でなく **`ok` と印字**する | **0 → 7節を足して 2** |
| ⑤ 全部戻す | 0 |

④ が 0 だった。上限の回の終了コードは細工しても 3 のままなので、1-6 節は全部緑を出す。
だが**読み手が見るのは行**で、其の行は「通った」と嘘を吐いていた。
終了コードは判定の**一部**でしかない。7節(出力の文言、5本)を足して 2 になった。

#### census 側は、写しの木で死なない形にしてから広げた

一覧を両言語へ広げると `readdirSync(join(ROOT,"..","ios","tools"))` が rc-backend だけの
写しで **throw = 赤**になる。1時間前に `f976012` で直した其のものなので `requireOutside` を通した。
更に自分の初版は skip 時に一覧から **落として**いた —— 落とすと TAP から名前ごと消えて
「測っていない」が誰にも見えなくなる。名前は残して個別に skip し、理由を言わせる形に直した。

実測: 完全な木 **726 / 726 緑・skipped 0** / rc-backend だけの写し **29 件 fail 0 skipped 8**
(8件とも「部分木の写しなので測っていない」と名指し)/ `live-send-check-control.sh` **pass=21 fail=0** /
`copied-tree-controls.sh` pass=4 / `no-linerefs-controls.sh` 11/0 / `mutation-freeze-controls.sh` 6/0 /
`shellcheck -S warning` 0 / `disposable-session.mjs limited` を使い捨て pane で実測(limited / not-limited / 登録簿に無い→1)。

★残り: `live-interrupt-check.sh` の枯渇枝は 3 と 2 に割ったが、**判定を駆動する対照はまだ無い**
(`live-send-check.sh` にしか真理値表を当てていない)。

#### commit の門が、私が回し直さなかった2件を捕まえた(同 23:5x)

上の一式を回した**後**に2つ手を入れて、回し直さずに commit した。門が止めた(726 中 2 件赤)。

| 落ちた検査 | 何が起きたか |
|---|---|
| `disk 上の live-* と一覧が一致している` | 新しい対照 `live-send-check-control.sh` が `live-` で始まるので、**7本目の計器**に見えた |
| `live-send-check.sh は 3 を返す道を持っている` | 判定を `send_verdict()` へ切り出した時、`exit 3` が `return 3` に変わって形が一覧に無くなった |

直し方は両方とも「検査の側」:
- census は `*-control(s).sh` を数えない(対照は計器ではない。対照の登録漏れを見るのは
  `tools/run-controls.sh` 側の照合で、そちらは私が同じ commit で足した)。
- `RETURNS_THREE` に `return 3` を足す。**緩めた訳ではない** —— shell 側の本当の証拠は
  対照の真理値表(終了コードが実際に 3 で出る事を 21 通り)で、静的検査が守るのは
  「3 へ行く道が1本も無い」台本だけ。

★教訓は「最後に触った後に回す」。門が無ければ、私は緑だった記憶のまま出していた。

#### 陰性対照で2回目の同じ失敗: **細工が当たった事を確かめてから読む**

`return 3` を潰す細工を撃ったら緑だった。穴かと思ったが、`sed` の綴りが
`^  return 3$`(空白2)で、本体の行は空白4だった = **細工が当たっていなかった**。
当たらなかった細工は、穴と**見分けが付かない緑**を出す。
昨夜は「細工を1つずつ戻してから次を撃つ」を学んだ。今夜はその前段 ——
撃った後に **hash が変わった事**と**狙った綴りが 0 行になった事**を先に見る。
確かめてから撃ち直したら 1 本赤くなった(`not ok 20`)。

### 2026-08-07 00:1x 割り込みの計器も、判定を切り出して真理値表を当てた(29 本)

前の節で残していた1件。`ios/tools/live-interrupt-check.sh` は本体を1回走らせても
**verified の枝しか通らない** —— already-done / not-in-flight / 陰性対照が壊れた時 /
上限は、全部**一度も走った事が無いまま**書かれていた。姉家族と同じ手で切り出した:

- `busy_exhausted_verdict <busy> <limited>` = 輪が尽きた時。3(上限)/ 2(仕込みの側)。
- `interrupt_verdict <rc> <分類語> <対照A> <対照B> <limited> [Bの終了コード]` = 最後の4つの足。
- 口は `--busy-verdict` / `--verdict`。**引数の輪より前**に置く(既存の `--classify` と同じ理由)。
- 対照 = `ios/tools/live-interrupt-check-control.sh`(29 本、実測1秒未満)。
  `.harness/live-interrupt-wording-controls.sh` とは別物 —— 向こうは**掴む文の一致**、
  此処は**掴んだ後の裁き方**。同じ台本の別の穴。

#### 順序が正本と違って見える所を、コードと対照の両方に書いた

`rc-backend/tools/exit-codes.mjs` は `2 > 1`。此の台本の最後の判定は **1 > 2**。
矛盾ではなく、**2 の意味が違う**: 正本の 2 は prepAbort(準備段で中断 = 下流が走っていない)で、
此の台本ではそれは 1-4 節がその場で返して最後の判定へ来ない。最後の判定の 2 は
「主の足が着地しなかった」で、陰性対照の2本は**それとは独立に測れている** ——
だから其処の赤は本物の欠陥。2 を勝たせたら、陰性対照が壊れた回を
「回し直せば直る」と報告する計器になる。5節が此処だけを撃つ(入替えると 10 本赤)。

#### 陰性対照⑤がまた 0 赤 —— 今度は**行頭の印**を見ていなかった

| 細工 | 赤 |
|---|---|
| ① 細工なし | 0 |
| ② 上限を無視(常に 2) | 1 |
| ③ 順序を入替(着地しなかった足が赤より強い) | 10 |
| ④ 訊けなかったを上限扱い | 2 |
| ⑤ 着地しなかった足を `ok` 印で書く | **0 → 印を見る2本を足して 1** |
| ⑥ 全部戻す | 0 |

⑤ の細工は行頭の `  --  : ` を `  ok  : ` に変えるだけで、**足の文は「測れていない」のまま**。
私の文言検査は足の文しか見ていなかったので素通りした。読み手が最初に読むのは行頭の印なので、
**印そのもの**を見る2本(在る事・通った回には出ない事)を足した。
同じ穴が姉家族の `live-send-check-control.sh` にも在ったので**同じ日に両方へ運んだ**
(そちらの細工は 2 赤)。教訓を N 箇所のうち1箇所にしか当てない型は、この節の主題そのもの。

実測: 対照 29/0(割り込み)・23/0(送信、2本追加後)/ 完全な木 726/726 skipped 0 /
未登録の対照 0 本(登録 61 本)/ `shellcheck -S warning` 0 / 細工は毎回 hash で当たった事を確認。

---

## 2026-08-06 01:0x — 「対照の無い判定道具 35 本」は 10 本だった、そして本当の穴は別の場所に在った

### 分母を2回間違えた

夜の続きで「終了コードで裁くのに対照が無い道具」を数えた。最初の答えは **33 本**、次が **35 本**。
どちらも嘘で、正しくは **10 本**だった。間違いの中身が2つとも別種なので両方書く。

| 版 | やった事 | 何を落としたか |
|---|---|---|
| 1回目 33本 | 宣言を `rc-backend/test/*.sh` から集めた | `test/` に居る **`.mjs` の宣言**を読んでいない |
| 2回目 35本 | `.mjs` を足した | 宣言の**綴りが2種類**在る事を知らなかった |

2つ目が本題。宣言には木ごとの基点が在る:
...
`tools/coldboot-chain.sh` と `rc-backend/tools/coldboot-chain.sh` を文字列で突き合わせれば当然
一致しない。**18 本が「対照が無い」側に落ちていた**。門(`tools/staged-controls-gate.sh`)は
`SCAN_SPECS` に `dir|基点` の対で持っていて正しく解決する —— 間違っていたのは私の写しの方。
2026-08-05 に「一覧を1つにして残り2箇所を導出」と書いた当人が、その日のうちに**4本目の写し**を
作った事になる。
...
| 道具 | 実際の守り |
|---|---|
| `exit-codes.mjs` | `test/live-exit-codes.test.mjs`(29 本)。`.test.mjs` は門の走査外なので宣言が付かないだけ |
| `live-{http,choice}-check.mjs` | 正本 `exitCodeFor` へ委譲済み。呼び方の綴りまで固定されている |
| `live-{fork,inject}-check.mjs` | 手写しだが**故意**。fork の `looksLimited` は正本との**包含関係**が測ってある |
| `serve-decision-check.sh` · `rc-backend-launch-check.sh` | 名前が `-control` でないだけの対照。前者は `verify-on-edith.sh` の検査段が回す |
| `ios/tools/{build,shots}.sh` | 建てる側の包み。判定は xcodebuild の結果そのもの |

### 途中で「本番の検査台本が fail-open」と誤診した

`verify-on-edith.sh` の採点行がこう見えた:

```
npm test 2>&1 | tail -12; rc=$((rc + $?))
```

パイプの後の `$?` は tail の物だから、赤い段が 0 に化ける —— **今夜私が `git commit | tail -60` で
2回踏んだのと同じ形**。5行在ったので「本番の検証が fail-open」と書きかけた。

誤り。**remote 台本の前置きが同じ罠を名指しして `set -o pipefail` を立てていた**。実測:
...
自分が同じ穴に落ちた直後だと、同じ形が全部穴に見える。**現物の前置きを読む前に5行を数えた**のが
順序の間違い。

### そこで見つけた本物 —— 1行の防御を、誰も見張っていなかった

fail-closed を保っているのは `set -o pipefail` の**1行だけ**で、
これを消しても repo は全部緑のままだった(実測)。防御は在るが、**防御が消えた事を知らせる物が無い**。

`test/verify-script-controls.sh` に4本足した(6 → 10 本)。文字列 grep ではなく、
**偽 ssh が受け取った現物の byte から前置きを取り出し**、故意に落ちる足を1本足して回す:

| | 見る物 |
|---|---|
| C7 | 採点される足が実在する(0 本 = 形が変わった = 測定不能。黙って緑にしない) |
| C7b | `tail` へ流す段の数 == `rc` に足す段の数(**6本目を足して採点を書き忘れた**回を捕まえる) |
| C8 | 落ちた足が合算に乗る(前置きが fail-closed を保っている) |
| C9 | 負の対照: 同じ前置きから pipefail を抜くと 0 に化ける(C8 が空振りでない証明) |

細工で確認(どちらも hash 変化と綴り 0 行を先に確かめた):

| 細工 | 結果 |
|---|---|
| `verify-on-edith.sh` から `set -o pipefail` を削る | **C8 が赤**(9/1) |
| 採点しない6本目の段を足す | **C7b が赤**(9/1) |
| 細工なし | 10/0 |

C7b が要る理由: C7 は「1本でも在るか」しか見ないので、他の5本が形に当たる限り
**6本目の書き忘れを素通しする**。1箇所だけ広げて広げたつもりになる型の、また同じ顔。

### 持ち帰り

- 分母を出す時、**鍵の綴りが1種類だと仮定しない**。門が対で持っているなら写しも対で持つ。
...
## 2026-08-07 — Sprint 6.5(spec §6 Day 7)の受入を、手書きでなく**道具の出力から**起こした

spec §6 の Day 7 は「REQUIREMENTS §5 のうち v1 該当分(#1-3 / #5-7 / #9)を証跡付きで確認」。
証跡の宛先も spec が名指ししている(`.harness/evidence-2026-08-1x/sprint6-acceptance.md`)。

手で表を書かなかった理由は、この file が §4-4 に自分で記録している形そのもの ——
**「配備した事を書く場所と、配備待ちだと書いた場所が別だった」**ので、
書いた事と測った事が別々に腐った。なので `.harness/dod-sprint-6.5.sh` を書き、
受入 md はその出力から起こした。

### 結果(`DOD_FULL=1`、終了コード 1)

**緑 12 / 赤 1 / 未測定 2**。

| 判定 | 行 | 中身 |
|---|---|---|
| 緑 12 | 0 / 1-a / 1-b / 2-a / 2-b / 3 / 5-a / 5-b / 6-a / 6-b / 7 / 10 | 一覧が先・続きが読める打てる・BUSY で干渉できる・切断を跨ぐ・待たされない・外部依存ゼロ・証跡そのもの |
| **赤 1** | 9(§5-9 期限性) | 日本に残す 2 台**とも**生きた鍵の期限(140 日 / **43 日 = 2026-09-19**) |
| 未測定 2 | 5-c / 6-c | 電話が要る。走らせ方を変えても緑にならない |

★10 行目は**この照合表が吐く受入 md 自身**を見る行なので、表を起こした1回目
(00:40)ではまだ存在せず未測定だった(緑 11 / 未測定 3)。md を書いた後の再走(00:52)で
緑になり、**他の 14 行は1文字も変わっていない**。両方の log を証跡 dir に残した ——
消すと「証跡が自分を測る行」がどう埋まったかを後から辿れなくなる。

### 唯一の赤は、実装の欠陥ではなく**渡米後に届かなくなる経路**

§5-9 の主張は「Tailscale 越しの純ソフト = 渡米後も作れる」。崩す条件は1つだけ ——
**日本に置いて行く機械の鍵に生きた期限が在る**事。切れた後の復旧はその機械の前なので届かない。
近い方の 2026-09-19 は `DESIGN.md` §8-5 の締切そのもの。渡米は 08-20 = **出る前に済む仕事**。

★初版は `--chain` でこの MBP も数えて赤を出していた。**この機械は Tom と一緒に渡米する**ので、
鍵が切れてもその場で入り直せる = 主張を崩さない。falsifier を「日本側だけ」に絞った。
§8-5 が名指しするのも日本側 2 台で、self は入っていない。

### log が「今の原稿」の話かを、代理でなく中身で測る様にした

`dod-sprint-6.5.sh` は「同じ1本の log に passed として名前が在る」で緑を出す。
その log がいつの中身の話かを、log 自身は何も語らない。代理を2つ試して**両方外した**:

| 代理 | 何故駄目か |
|---|---|
| mtime | `dod-sprint-6-controls.sh` は変異を植えて複製から戻す。戻した瞬間**中身は同一なのに mtime だけ新しい** = 恒常的に「古い」と読む(初回に実際にそう出た。4 本が新しいと言われ、git は clean) |
| commit 時刻 | 検査は commit の**前**に走る。log が commit より古いのは正常な順序で、異常と読むと毎回未測定 |

→ `ios/tools/build.sh --sim` が走る**前**に原稿の指紋(Sources/Tests/UITests の全 .swift の
`shasum` を畳んだ物)を `ios/build/xcodebuild-sim.sources.sha` に落とす様にした。
照合側は今の指紋と突き合わせる。指紋の無い log(古い build.sh 製)は**未測定**であって緑ではない。

### 負の対照 14 / 14 —— そして対照が照合側の欠陥を 3 つ捕まえた

`.harness/dod-sprint-6.5-controls.sh`。**作業木は一切触らない**(照合表が読む file だけを
scratch に複製して壊す)。走らせる前後で `git status` が同一である事を観測して確かめている。

| 捕まえた欠陥 | 何が起きていたか | 直し方 |
|---|---|---|
| **指紋の判定が下の行へ伝わっていない** | 0 行目は指紋を見ていたのに、log を根拠にする 5 行は 0 行目を見ていなかった。**註には書いてあり、機械は何もしていなかった** | 0 行目が緑の時だけ上がる旗を置き、log 頼りの行が全部それに従う様にした |
| 6-a の定数照合が**部分一致** | `static let writeTimeoutDisabled` が `writeTimeout` の在る証拠として通る = **定数を殺す改名を緑で素通し** | 語の境界を付けた |
| 1-a が対象を見失うと**黙って緑** | 「含まない」を根拠にする行は、抜き出しが空でも「含まない」が真になる | 抜き出しが空なら未測定へ落とす |

1件目を直す前と後で、指紋を壊して直に測った(scratch の複製、作業木は無傷):
...
★これを見落としていた理由が要点: 対照 1・2 は**0 行目しか見ていなかった**。
0 行目が未測定になる事は測ってあり、対照は緑だった —— その隣に、同じ指紋に依存しながら
対照の無い経路が 5 本並んでいた。**対照が緑でも、対照の無い経路が隣に在る**。

★対照の側も1つ外していた: `normalFlow` を `normalFlowRenamed` に改名しても目印は部分一致で
残るので、抜き出しは成功したままだった = **空振りの状況を作れていなかった**。
目印を**消す**改名(`mainRoute`)に直して初めて赤が出た。
—— 対照が本物である事も、対照だけでは確かめられない。

★もう1つ、対照の判定を読む側が知っている必要が在る事: **build と同時に走らせると偽の NG が出る**。
`DOD_FULL=1` の照合と並走させたら、複製した log と指紋が書き込み途中の物になり、
基準が緑にならない行が出て OK 13 の内容のまま終了コードだけ 1 になった。対照の頭に書いた。

| 走らせた物 | 結果 |
|---|---|
| `DOD_FULL=1 bash .harness/dod-sprint-6.5.sh` | 緑 11 / 赤 1 / 未測定 3、exit 1(赤は §5-9 = Tom の操作待ち) |
| `bash .harness/dod-sprint-6.5-controls.sh`(単独で) | **OK 14 / NG 0**、exit 0、作業木の差 0 行 |
| `bash rc-backend/tools/run-controls.sh`(全掃き 59 本) | green=58 / red=1 / 未測定=0。赤は **私が作った物**(下記)、直して緑 |
| headless sim(`ios/tools/build.sh --sim`) | exit 0、log 最大 **370**(単体)+ UI 7 = 生 `passed` 377 本 |
| 引用した検査名 | **21 種、全部実在**(受入 md 側も照合行で緑) |

### 全掃きが捕まえた、私が作った赤 1 本

新しい対照を `rc-backend/tools/run-controls.sh` へ登録した序でに全掃き(59 本)を回したら
`no-linerefs-controls.sh` が赤。原因は**この夜に自分で書いた註**だった ——
`ios/tools/build.sh` の中で `dod-sprint-6-controls.sh` を backtick 引用したが、
その file は `.harness/` に在るので `ios/tools/` から見て解決しない。
`no-linerefs` は「backtick で引いた名前が実在するか」を見る門なので、正しく赤にした。
`.harness/` を付けて緑。**引用は書いた瞬間に腐る**ので、腐りを見る門を持っている事の値打ち。

### 持ち帰り

- **証跡は「書く」物ではなく「起こす」物。** 手で書いた表は、測った物と別々に腐る。
  受入 md を道具の出力から起こしたので、次に走らせた時に食い違えば表の側が赤くなる。
- **「今の木の話か」を mtime や commit 時刻で代理しない。** どちらも中身の代理として成立しない事を
  実際に測って確かめた。中身を測る道具が無いなら、**作る側(build.sh)に指紋を吐かせる**。
- **対照が緑でも、対照が効いているとは限らない。** 接尾辞を足す改名は、部分一致の目印を壊さない。
  「壊した」と思った操作が本当に性質を壊しているかを、対照ごとに1回疑う。
- **註は守りではない。** 「0 行目が未測定なら下は全部古い木の話」と自分で書いておきながら、
  機械は何もしていなかった。書いた本人には**書いた記憶**が残るので、一番気付けない形で穴が開く。
  この repo が繰り返している「守りを記憶に懸ける」と同じ形 —— 註を書いたら、その註を**測る**。
- **対照が緑の隣に、対照の無い経路が在る。** 指紋の対照は 2 本とも緑だったが、両方 0 行目しか
  見ていなかった。同じ指紋に依存する 5 行には対照が無く、そこが穴だった。
  次に対照を書く時は「この性質に依存している行を全部数えたか」を1回問う。
- **赤を1本残して閉じてよい。** §5-9 は実装の欠陥ではなく Tom の 1 操作で、期限が渡米日より後。
  緑に丸めず、締切と行き先(`DESIGN.md` §8-5)を付けて赤のまま出す方が使える。

---

## 2026-08-07 — 門が名指しした穴を塞ぐ: 指紋を**作る**側の対照

前の commit(`86bf829`)の門は、止めはせずにこう注記していた:

```
staged-controls-gate: 注記 — 対照を導けない道具: build.sh
```

放置しなかったのは、`build.sh --sim` が落とす**原稿の指紋**が受入の緑 6 行
(0 / 1-b / 2-b / 3 / 5-a / 6-b)の根拠そのものだから。指紋を**読む**側
(`dod-sprint-6.5.sh`)には対照が 14 本在るのに、指紋を**作る**側は誰も見ていなかった。

### 壊れ方は2種類あり、危ないのは片方だけ

| 壊れ方 | 0 行目 | 危険か |
|---|---|---|
| 指紋が書かれない / 壊れる | 未測定 | いいえ(閉じる側に倒れる) |
| 指紋が**古い中身**を指す | **緑** | **はい** —— 古い log が今の木の証拠になる |

だから測るのは「在るか」ではなく **順序**(xcodebuild より前に取っているか)と
**範囲**(Sources / Tests / UITests のどれを変えても動くか)と **中身か mtime か**。
指紋の式は build.sh から切り出して回す(写しを持たない)。xcodebuild は回さないので 1 秒未満。

### 結果 — `.harness/build-fingerprint-controls.sh`(9 本、全部緑)

陰性対照(変異を build.sh へ植える)。狙った行だけが赤くなる:

| 変異 | 赤くなる行 |
|---|---|
| 無変異 | なし(9/9 緑) |
| 指紋を xcodebuild の**後**へ移す | 順序 1 本 |
| 範囲から UITests を落とす | UITests 1 本 |
| 中身 -> mtime | 中身 3 本 + mtime 1 本 |

証拠: `.harness/evidence-2026-08-1x/build-fingerprint-controls.log`

### 書きながら自分で作った赤 3 本(全部この対照自身が捕まえた)

1. **戻しが不発だった。** `$(case $f in A) echo 1;; …)` の `)` が command substitution を
   先に閉じ、戻しの行が構文エラーで落ちていた。捕まえたのは
   「戻すと指紋も戻る」の行 —— 戻し損ないは**後続の対照を別の理由で赤くする**ので、
   戻せた事自体を測る行が要る、が此処の教訓。以後は複製から戻す。
2. **mtime の対照が、緑の理由を取り違えていた。** 素の `touch` は作った直後だと同じ秒に
   落ちる。だから式を mtime 版に差し替えても此処は緑のままだった。**壊して赤くなるかを
   試すまで、緑は緑である事しか意味しない。** 今は `touch -t` で明示の時刻を打つ。
3. **陰性対照の表が再現しなかった。** 同じ変異を2回回して NG 4 と NG 2 が出た ——
   秒の境目を跨いだかどうかで中身の行が結果を変えていた。木全体の mtime を固定して、
   中身の行が**中身だけ**を測る様にした。更にその後、後始末を怠って
   「変異のせいではなく私が打った別時刻のせい」で赤くなる偽の赤を 1 本出し、
   各節が木を canonical へ戻す形にして消した。

### 持ち帰り

- **門の注記は、止めない事と、無視してよい事は別物。** 「対照を導けない道具」は
  1 行の情報だが、指していたのは受入の緑 6 行の土台だった。止めない警告こそ読む。
- **陰性対照の表は、再現するまで表ではない。** 1 回回して得た kill set は仮説。
  同じ変異を2回回して同じ物が出て初めて、道具の性質を測ったと言える。
- **対照が偽の赤を出す一番多い原因は、変異ではなく自分の後始末。** 節をまたぐ時は
  木を canonical へ戻す。戻したかを測る行も置く。

## 2026-08-07 — 対照の集合を「名前」ではなく「宣言」で決める

### 何が起きていたか

`build.sh` の指紋対照を入れた直後、同じ形の**より大きい穴**が見つかった。
`rc-backend/tools/` に居る2本 —— `serve-decision-check.sh` と
`rc-backend-launch-check.sh` —— は中身も註も紛れも無く陰性対照(前者は全19ケースを
旧判定にも通して退行を機械で捕まえる、後者は `tailscale` と `node` を偽物に
差し替えて起動ラッパを実際に走らせる)。にも関わらず commit の門はこの2本を
**ただの道具**として扱っていた。名前の末尾が -check.sh で、置き場が走査対象外の
`tools/` だったから。

実測された帰結: **`rc-backend-launch.sh` を直す commit は、対照を1本も回さずに
通っていた。** その道具の documented な壊れ方は「edith の上では全部緑、電話からだけ
永久に到達できない」—— 緑が届かない側を見張る唯一の対照が、回っていなかった。

根は 2 本の一覧が食い違っていた事。門は「対照か」を `*-control*.sh` という**名前**で
決め、「何を見張るか」を `# controls-for:` という**宣言**から読んでいた。
この repo が註に6回書いている「手で同期する一覧を2本持たない」に、門自身が
違反していた。

### 捨てた案(どちらも measurement で、assumption ではない)

| 案 | 実測した帰結 |
|---|---|
| `rc-backend/tools` を**名前で**走査に足す | `prove-control.sh` `prove-all-controls.sh` が `*-control*.sh` に当たる。宣言が無いので `undecl` が発火 = 門が自分の道具を対照だと言い出して commit を止める |
| glob を `*-check.sh` まで広げる | `ios/tools/live-interrupt-check.sh` `live-send-check.sh` を巻き込む。この2本は edith が要る = 手元の commit で必ず倒れる |

どちらも「広げる」方向で、名前で決めている事自体は直っていない。採ったのは根治:
`SCAN_SPECS` に3つ目の欄(拾い方 = `name` / `decl`)を足し、道具と対照が同居する木では
**宣言を持つ file だけ**を対照にする。一覧は `SCAN_SPECS` 1 本のまま。

### 測った物

| 何を | 結果 |
|---|---|
| orphan(対照を導けない道具)の列挙 | 13 本 → **9 本**。消えたのは狙った4本ちょうど |
| `--would-select rc-backend-launch.sh` | `rc-backend-launch-check.sh` |
| `--would-select serve-decision.sh` | 上記2本の両方(1つの道具を2本が見張る形も通る) |
| `serve-decision-check.sh` 単体 | exit 0 / 0 秒 / 旧と答えが違った行 14 件 |
| `rc-backend-launch-check.sh` 単体 | exit 0 / 4 秒 |
| `staged-controls-gate-controls.sh` | **PASS 59 / FAIL 0**(3.4 秒)。S49-S52 が新規 |

新しい4行は両向きに測る。S49 = 名前が対照らしくなくても宣言が在れば回る。
S50 = 同じ木の**宣言を持たない** `*-control*.sh` を staged にしても門は止まらない。
S51/S52 はその2行に歯が在る事の陰性対照で、変異を当てた事自体も `cmp` で先に確かめる:

- S51: 宣言で弾く1行を消す → `prove-control.sh` を staged にした commit が **exit 1** で
  止まる。捨てた案①が本当に全 commit を落とすと、手ではなく機械で確定した。
- S52: 走査先の1行を消す → 砂場の gg-check.sh が**黙って**回らなくなり、門は
  「触れた対照は無い」と言う。此処が一番危ない壊れ方(赤ではなく緑で失われる)。

### 持ち帰り

- **「対照である」と「何を見張るか」を別々の情報源から決めない。** 名前で集めて宣言で
  読む形は、両者がずれた瞬間に**発見されない対照**を作る。門は正直に
  「対照を導けない道具」と言い続けていたが、それは穴が無い事の証拠ではなかった。
- **止める側の壊れ方より、黙る側の壊れ方を先に測る。** S51(止まりすぎる)は commit の
  たびに気付く。S52(回らない)は誰も気付かない。歯を入れるならまず後者。
- **設計判断は、捨てた案も走らせてから捨てる。** 上の表の2行は推論ではなく実行結果で、
  だからこそ S51 として対照に落とせた。「たぶん壊れる」は対照にできない。

## 2026-08-07(続き)— 注記から**紛れ**を落とす(穴ではなく)

前の節を commit した直後、同じ注記をもう一度読んだ。名指しされていた 13 本のうち
4 本が本物の穴で、残り 9 本は実機計器 —— 生きた pty や simulator が要り、commit 時に
回せる物ではない。**穴と既知が同じ字面で並んでいた。** 注記は 08-02 から 08-07 まで
5日間ずっと正しく4本を名指ししていたのに読み飛ばされた。

### 自分の案を先に叩いた

免除の語彙を足す案には強い反対がある: **警報が死ぬ典型的な経路がこれ**。
以後すべての道具に `no-control: 計器だから` が貼られて注記は永久に黙る。しかも
今回の4本は「紛れて見えなかった」のではなく「正しく名指しされていたが読まれなかった」
—— 計器の欠陥ではなく読み手の欠陥だ、という読み方も成り立つ。

採った理由は detectability の差: 9 本を降ろすと、**10 本目の本物の穴は一覧で唯一の名前**に
なる。10 番目ではなく唯一。ただし判子を防ぐ条件を2つ課した上で:

| 条件 | 形 |
|---|---|
| 理由は 12 字以上でないと数えない | 短い理由は免除にならず**名前が残る**。門は止めないので、判子を押しても何も買えない = 自己強制。第2の一覧も新しい hard-fail も要らない |
| 免れた本数は常に出す | 名前が消えた事を「0 本」と読ませない。`注記 — 理由を宣言して免れた道具 9 本` |

### 9 本に書いた理由(判子にしない為、種類を分けた)

- **実機計器 7 本**(`live-*` 3 / statusline 系 2 / spinner 1 / `shots.sh`)= 生きた pty・
  simulator・server が要る。うち statusline 系と spinner は**期待値を持たない**
  (字面を撮るだけ)ので、そもそも対照ではない。
- **`exit-codes.mjs`** = 純粋関数で、単体 `test/live-exit-codes.test.mjs` が commit ごとに
  回っている。免除の理由が他の8本と**違う**。
- **`make-icon.py`** = 出力の PNG を commit 済みで、作り直すのは図案を変える時だけ。
  対照を置くより目視が安い、という**費用対効果の判断**。計器だからではない。
  此処を「計器」と書いたら、それが判子の第一号になる。

### 測った物

| 何を | 結果 |
|---|---|
| 157 本の道具全部を staged にして門を撃つ | 名指し **0 本** / 免れた **9 本** |
| `make-icon.py` の理由を `x` に潰す | 免除されず**名前が戻る**(判子は買えない) |
| `staged-controls-gate-controls.sh` | PASS **62** / FAIL 0(S53-S55 が新規) |

S55 は床の歯: `-ge 12` を `-n` に潰すと判子が通る = S54 が字数を測っている事の確認。

### 持ち帰り

- **警報を静かにする改修は、静かにした分だけ「何本静かにしたか」を出す。** 名前を消して
  本数も消したら、それは改善ではなく隠蔽になる。
- **免除は理由の**種類**を書かせると判子になりにくい。** 9 本のうち 2 本は他と理由が違い、
  書く時にそれを言語化して初めて「`make-icon.py` は計器ではない」と気付いた。

---

## 2026-08-07 — 渡米前の実測: 本番 edith と、電話のコードを本物のサーバへ当てる

対照の整備を3本続けたので打ち切り、**残る本当の危険**へ移った。アプリは仕様上
機能が揃っている(Sprint 6.5 が最後、受入は緑)。渡米まで 13 日で残るのは
「**サーバの側が上がり続けるか**」と「**電話のコードが本物のサーバで動くか**」の2つ。

### 1. edith の自力復帰 —— 既に在る道具を回す(自分で測り直しかけた)

手で `fdesetup` / `defaults` / `pmset` を叩いて7つ確かめた後で、**同じ7つを測る道具が
既に在る**事に気付いた(`rc-backend/tools/coldboot-chain.sh`、対照 17 本)。
「新しく作る前に既存解を1回だけ安く確かめる」を踏み外した形で、**手で測った値は捨てて
道具の判定を正本にした**。道具は「見たい機械の上で走らせる」設計なので edith の上で実行。

| 何を | 結果 |
|---|---|
| `coldboot-chain.sh` を edith の上で | **7/7 ・ exit 0**(鎖は繋がっている) |
| 3回の再起動(7/26・8/04・8/06)の復帰 | **3回とも人の手なし** |
| launchd `com.edith.rc-backend` | pid 574 / last exit 0 / KeepAlive・RunAtLoad = true |
| error log 2本 | **在って 0 byte**(`ls -la` で確認。空と不在は別物) |
| `serve-decision.sh 8787` | `ok`(tailnet の入口は再起動を跨いで生きている) |

### 2. 自分の仮説が実測で外れた

~2日おきの再起動を **macOS の自動更新だと踏んだ。違った** ——
`softwareupdate --history` の最後の導入は **2026/03/20**、7月・8月は 0 件。
`LastFullSuccessfulDate` は再起動の **3分後** = 原因ではなく起動後の確認。
再起動の原因は**今も判っていない**(panic 0 件 / `pmset -g log` に停止理由が無い /
unified log は再起動より前を持っていない)。6手掛けて判らなかったので打ち切った ——
**復帰の仕組みが判った時点で、原因は打つ手を変えない**から。

見つかったのは別の物: **macOS Tahoe 26.6 が保留 + 自動導入が on**。危ないのは再起動
その物ではなく、**更新が自動ログインを解除する形**。解除されてもその場では何も壊れず、
**次の再起動まで黙って武装する**。→ DESIGN §8-11(Tom の判断。sudo が要る)。

**監視を足すのはやめた**。武装を検知する日次の job は書けるが、更新を止めれば原因ごと
消えるので、その場合ほぼ何も守らない番人になる。渡米 13 日前に鳴る経路を1本増やす方が、
yoda で実際に起きた「鳴り過ぎて経路ごと黙らせる」に近づく。**症状ではなく原因を取った**。

### 3. 電話のコードを本物の rc-backend へ当てた(2本とも初めて緑を観測)

| 検査 | 結果 |
|---|---|
| `ios/tools/live-send-check.sh` | **exit 0**。転写の行数 0 → 8 / 本文の一致 1 件 / 陰性対照 0 件 |
| `ios/tools/live-interrupt-check.sh` | **exit 0**。`stopped:"verified"` の文が届いた |
| 陰性対照A(止まった後にもう一度撃つ) | `not-in-flight` の文 = **verified を偽らない** |
| 陰性対照B(でたらめな鍵) | **401** = モックではなく本物のサーバに当たっている |
| 後片付け | セッション / panes json / 転写 の**3つとも不在を確認** |

割り込みは**当て推量の n 秒で撃っていない**: 生成中を spinner で観測してから撃つ。

★正確に書く: 本体の註が言う「一度も走った事が無い」は**枝**の話で、道具全体ではない。
1回の実機走行で通るのは `verified` の枝だけ、という意味。今日の走行は陰性対照Aが
`not-in-flight` を出したので、**4枝のうち2枝**に実機の裏が付いた。残り2枝
(`already-done` / 上限)は今も真理値表の対照だけが持っている。

### 持ち帰り

- **手を動かす前に「同じ物を測る道具が既に在るか」を1回引く。** 今回は7つ手で測り切った
  後に既存の道具が出てきた。安いのは順序を逆にする事だけで、費用は毎回同じ。
- **原因が判らないまま打ち切ってよい条件は「原因が打つ手を変えない」時だけ。** 再起動の
  理由は不明のままだが、復帰の**仕組み**(FileVault off + 自動ログイン → LaunchAgent)が
  判ったので、次の一手は変わらない。判らない事は判らないと書いて残す。
- **番人を足すのが常に善ではない。** 原因を消せる時に症状の監視を足すと、守らない番人が
  1本増える。番人の費用は「鳴った時に人が黙らせたくなる確率」で数える。

---

## 2026-08-07 — 止まる前に残っていた自前の仕事(繰り越し3件のうち2件を塞ぐ)

`authorize-stop` が REFUSED を返した。理由は2つで、片方は**私には満たせない**:

| 門の言い分 | 実際 |
|---|---|
| manifest の3件は「門が未処理だと思っていない」 | manifest を `obligations` の出力から組んだのが誤り。門が自分で数える物と揃っていない |
| queue が空なのに直近2時間の REPLAN 記録が無い → replan せよ | `check` は **replan 予算 2/2 消費済み**で STOP を返す。**この2つは同時に満たせない** |

ここで待ちに入るのは誤り。矛盾しているのは**門の2つの指示同士**であって、
「仕事が尽きた」とは別の話だからだ。繰り越しの自前作業が現に残っていた。

### 1. handoff が session と一緒に消えるパスを引いていた

`.harness/spec-native-shell-2026-08-05.md` が先行版(113行)の置き場として
session の scratchpad の**絶対パス**を引き、「保全済み」と書いていた。あの dir は
session 終了で消える = **その一文は消えた瞬間に嘘になる**。渡米中に電話から読む
文書としては、死んだパスを指しているのが一番効く形の壊れ方。

`.harness/spec-native-shell-2026-08-05-superseded-113.md` へ複製(`cmp` 差分なし)。
**banner を足さず1バイトも変えていない**: 註を足すと §0-1 の差分の主張を原本に対して
検められなくなる。superseded である事は file 名と引用側の本文に持たせた。

★`no-linerefs` は 13/13 緑だが、あの門の走査木は `rc-backend + ios` で **repo 直下の
`.harness/` を見ていない**。緑を根拠にせず `ls` で直接確かめた。**緑の適用範囲を
先に読む**、が今日もう一度出た形。

### 2. 壊れた時の表示だけ、検査が1本も無かった

v1 の4機能は実機で全部緑。だが緑なのは**happy path だけ**で、`UnreachableBanner`
(届かない時に Tom が読む文)には検査が無かった。`detail` が `private` で、
SwiftUI の `body` は単体検査から読めない為。

`detailText(failures:context:)` を切り出して2層で検査を置いた。①文言そのもの
②**原因を名指ししていない事**。②が本体 — ①だけなら「言い換えついでに元の欠陥を
戻す」編集が通る。この view の doc が記録している通り、前の版は
「バックエンドに接続できません」で始まっていた = 電話が区別できない3原因
(電波 / edith 停止 / tailnet 切断)の1つを**事実として断言**していた。
②には陽性アンカーを付けた(禁止語リストは当たる事を証明しない限り無意味)。

変異で確かめた: `case .list` を旧欠陥に戻すと **7件中4件が赤**。同じ変異で
**既存377件は1件も落ちなかった** = これが埋めた穴そのもの。

### 3. 健康監視は「在るのに武装していない」— 武装しない判断

`health-observer.sh` はどこにも仕掛かっていない(launchd / LaunchAgents / crontab を
実測、該当なし)。武装しなかった理由:

- 観測者は edith 以外で走る必要がある = 実質 **Jervis**。Jervis は Tom の鞄の中で、
  蓋が閉じている間は走らない。**Tom が見ていない時にちょうど止まる**番人になる。
- それは「分からない」を「知らされたはずだ」に変える。**被覆率を測っていない番人は
  無いより悪い**。
- 捕まえる障害は元々自己申告する(電話を開けば出る)。しかも復旧手段の ssh は §8-5 に
  依存していて、そこは既に最優先で Tom に出してある。

### 持ち帰り

- **門が矛盾した2つの指示を出したら、それは「止まってよい」の合図ではない。**
  矛盾しているのは門の内部であって、仕事の有無とは無関係。繰り越しを読み直す方が先。
- **緑の適用範囲を、緑を使う前に読む。** 「13/13 緑」は走査木の外側を何も保証しない。
- **番人を足す/足さないは、被覆率を測ってから決める。** 「在った方が安心」は費用計算では
  ない。今回は観測者の稼働時間が Tom の不在時間と**逆相関**していた。

### 追記 — Planner の提案1件を証拠付きで却下(2026-08-07)

replan を回した(`check` は予算切れ STOP だが、停止の門が「REPLAN の記録が無い」と
言うので Planner は起こした)。提案は1件 `wire-e2e-into-run-controls`。**却下**。

| Planner の主張 | 実測 |
|---|---|
| e2e は「人が手で打った時しか走らない」 | `tools/verify-on-edith.sh` の `--- e2e ---` の段で走り、更に **deploy の度に** `tools/deploy-to-edith.sh` が staged 側で走らせている |
| 新しく `-controls.sh` の薄い殻が要る | 既に在る。`test/env-death-controls.sh` の `controls-for:` 行が `test/e2e-local.mjs` を名指しで宣言済 |
| `run-controls.sh` の配列に足すべき | あの配列は**対照**の一覧で、検査の一覧ではない。e2e の対照は既に `EDITH_CTLS` に居て、edith でしか意味が無いので既定 skip・`--all` で回る(呼び口の註が「edith 上でしか意味が無い2本は既定では回さない」と明記) |
| `npm test` から漏れている | `npm test` = `test/**/*.test.mjs`。92秒の統合検査が単体に居ない事は正しい |

★持ち帰り: **subagent の結論は入力であって判定ではない。** 提案の前提4つのうち4つとも
現物と食い違っていた。実装する前に呼び出し元を grep する費用は数十秒で、
実装してしまった場合の費用は「意味の無い対照が1本増える」= 対照の一覧の信頼が落ちる。

★同じ turn で**自分の計器も読み違えた**: `survival` を `grep -oE "…alive|dead…"` に通して
「dead 1 / DRIFT 6」と読んだが、あれは要約行の `dead=0` という**文字列**に当たった
出現回数だった。素で読めば `checked=10 alive=10 dead=0 drift=0` = 全部生きている。
**欠落のある filter の出力を意味として読まない。** 数える前に生の行を1回見る。
