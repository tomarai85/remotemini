...
測った日: **2026-08-09 22:14**(初版 = 2026-08-07 00:44。下の「測り直した理由」を参照)
測った物: `.harness/dod-sprint-6.5.sh`(`DOD_FULL=1`)
負の対照: `.harness/dod-sprint-6.5-controls.sh`
生の出力: `dod-sprint-6.5-2026-08-09.log` / `dod-sprint-6.5-controls-2026-08-09.log`
(初版 = `dod-sprint-6.5-full.log` / `dod-sprint-6.5-after-artifact.log` /
`dod-sprint-6.5-controls.log`。全部同じ dir に残してある)
...
**緑 12 / 赤 1 / 未測定 2**(終了コード 1)。判定の内訳は初版(08-07)と同じで、**数字だけが動いた**。

★**測り直した理由**(2026-08-09)。08-07 以降に電話側の原稿を S8-11〜S8-30 で大きく触ったので、
0 行目の指紋が合わなくなった —— 記録されていた log の指紋 `c0afecab2387` と今の原稿
`819347ba6a88` が違う = **その log は今の木を説明していない**。この状態で表を回すと
0 行目が未測定になり、log を根拠にする 5 行(1-b / 2-b / 3 / 5-a / 6-b)も道連れで未測定へ落ちる
(§4 の 0 番で直した伝播が、設計通りに働いた)。だから `DOD_FULL=1` で単体一式を回し直して
指紋を録り直した。**古い緑を書き換えたのではなく、古い緑が未測定に落ちたので測り直した**。
...
|---|---|---|---|
| 0 | 照合の土台 | 緑 | log 最大 **491 件** / 失敗 0 / 原稿の指紋 `819347ba6a88` が一致 |
...
| 9 | #9 期限性(渡米後も作れる) | **赤** | 日本に残す 2 台**とも**生きた鍵の期限が在る(137 日 / **40 日**)。→ `DESIGN.md` §8-5 |
...
| 日本に残す機械 | 残り | 期限日 |
|---|---|---|
| 1 台目 | 137 日 | 2026-12-25 |
| 2 台目 | **40 日** | **2026-09-19** |
...
★**この鍵は Tailscale の管理画面からしか無効化できない**(2026-08-09 に機械で叩いて確認)。
edith 側に `~/.config/tailscale` は無い / edith と Jervis の dotfiles・config・`~/.claude` に
API 鍵の名前(`tskey-api` / `TAILSCALE_API` / `TS_API_KEY`)が1件も無い / macOS keychain に
`tailscale` の項目が無い / CLI(1.94.2)には `tailscale set` の期限系 option も
`device`・`admin`・`api` の下位命令も無い。**4 段とも塞がっている**ので、これは
「調べずに Tom へ投げた」ではなく実測の上での Tom-gate。
...
| ViewModel から送り手を外す | 2-a が赤 | OK |
| 送り手を**保持するだけで呼ばない** | 2-a が赤 | OK |
| 容器が**本物の送り手を挿さない** | 2-a が赤 | OK |
