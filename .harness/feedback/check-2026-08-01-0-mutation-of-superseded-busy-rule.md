...
> ★2026-08-01 追記: この記録が検査していた BUSY 規約そのものが、同日の実測で無効になった。
> `esc to interrupt` はこのビルドの画面に存在せず(240 枚中 0)、BUSY は一度も発火していなかった。
> つまり下の負の対照は「実在しない状態の判定」を厳密に測っていた。現行の設計と検査記録は
> `check-2026-08-01-1.md` と DESIGN.md §2.9 ★2026-08-01 を見ること。
