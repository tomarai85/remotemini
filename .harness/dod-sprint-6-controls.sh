...
#
# 既知の費用(黙って隠さない): xcodebuild を 14 回(基準1 + 変異13)。所要は**単一の値ではない**。
#   実測(2026-08-06、どちらも門から呼ばれた本番):
#     同じ commit に ios/Sources を含む -> **1651s**(27.5 分)。DerivedData が無効化され、
#       基準ビルドが素から。以降の 13 変異も巻き添えで重い。
#     .harness/*.sh だけの commit      -> **247s**(4.1 分)。増分が効き、変異は 1 file ずつ。
#   ★同じ日に **11 分 -> 27 分** と直し、その次の走行が 4.1 分だった。
#     一つの誤った単一値を、別の単一値に置き換えていた —— **1 標本で「前後」と書くな**。
#     費用は「この commit が ios/Sources を触るか」で決まるので、そう書く。
#   ★見積りが外れた時の実害はこれ: 11 分を信じて `git commit` を 2 分・9 分20秒の
#     timeout で 2 回切り、2 回目の SIGKILL が変異を作業木に取り残した。
#     **待ち時間の見積りは、待つ側の道具の設定値になる。ずれた見積りは中断を作る。**
#     だから運用側の規則は数字に依存しない形にする: **この門を待つ commit は前景で
#     timeout を掛けず、背景で回して終了を待つ**。上限を当てられなくても壊れない。
...
# ★$IOS(先頭で絶対解決済み)から引く。相対 path 起動 + cd で空振りするのを避ける。
. "$IOS/tools/sim-device.sh"
