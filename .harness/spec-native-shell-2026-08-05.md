...
| 除外 | 根拠 | 本 spec での扱い |
|---|---|---|
| push 通知 | wildcard provisioning profile は push entitlement を運べない(team-lead 指示) | **★2026-08-12 に別経路で出荷済(下の訂正2)**。APNs は今も取れないが、要件は APNs を指定していない |
| アカウント表示・切替 | team-lead 明示除外 | **★2026-08-12 に出荷済(下の訂正2)**。§7 が自ら「静かに落とすと明記要件を無断で削る失敗」と書いていた |

**訂正2(2026-08-12、出荷時)**: 上の2行はどちらも **v1 に入れた**。理由:

1. **アカウント表示・切替** —— §7 は此れを v2 候補に置きながら、同じ表の中で
   「REQUIREMENTS 側は必須要件として記録済みであり、**静かに落とすと明記要件を無断で削る失敗**に
   なる — team-lead の再確認を推奨」と書いていた。再確認は来ないまま、着手コストは
   §7 自身の見積りで「低。サーバ実装済み。UI 側はラベル+ボタン1つ」。**除外の根拠より、
   除外が壊す物の方が大きい**。実装は `ios/Sources/Screens/Shared/AccountBar.swift`。
   ★実装して分かった訂正: 「ラベル+ボタン1つ」は**足りない**。切替は艦隊全体の状態を
   変えるので確認を挟み、更に「失敗しても口座は進んでいるかもしれない」という
   サーバ側の非対称(`server.mjs` の `/api/account/next` の注釈)を UI が引き受ける必要が在る
   —— 撃ち直さず**読み直す**。見積りが低かったのは此の非対称を数えていなかったから。

2. **入力待ちの通知** —— 除外の根拠は「wildcard profile は push entitlement を運べない」で、
   **之は今も真**。だが要件(REQUIREMENTS §5-4)が求めているのは
   「入力待ちで電話に通知が来て、開いてその場で返せる」であって、**APNs ではない**。
   実測(2026-08-12): edith には既に ntfy の鎖が全部繋がっていた —— `~/.ntfy-topic` が在り、
   `Notification` hook が `~/fleet-tools/ntfy-notify.sh` を指し、dry-run が通る。
   欠けていたのは**押した時の行き先**だけで、通知は ntfy のアプリが配送するので
   entitlement は1つも要らない。`Click: remotemini://` を足し、アプリに URL scheme を
   登録した(`ios/project.yml` の `CFBundleURLTypes`、焼き上がりの検査は `ios/tools/build.sh`)。
   ★**一般則**: 除外の根拠が「或る手段が取れない」の形をしている時、それが**要件の否定**に
   なるのは要件が其の手段を名指ししている時だけ。名指ししていなければ、塞がれたのは
   経路であって要件ではない。
   ★**残る Tom ゲート**: Tom の iPhone が ntfy の topic を購読している事(アプリを入れて
   1回購読するだけ)。之は物理デバイスの操作なので私からは動かせない。
...
|---|---|---|---|
| ~~**アカウント表示・切替**~~ | — | **★2026-08-12 に v1 として出荷済。此の表から外れた**(§1-a 訂正2)。実装 = `ios/Sources/Screens/Shared/{AccountBar,AccountViewModel}.swift` + `ios/Sources/Core/AccountClient.swift` | — |
| ~~**push通知(入力待ちを電話へ通知)**~~ | — | **★2026-08-12、APNs ではない経路で v1 として出荷済**(§1-a 訂正2)。APNs の entitlement が取れない事は今も真だが、要件は APNs を名指ししていない。配送は ntfy(edith 側に既設)、押した時の行き先が `remotemini://`。★残る Tom ゲート = iPhone で ntfy の topic を1回購読する事 | — |
...
~~push通知が取れない間の緩和策として…~~ **★2026-08-12 に不要になった**(上記)。
此の段落が提案していた「フォアグラウンドの間だけのハイライト」は、自ら書いている通り
**ロック中に気付けない**ので合格条件を満たさなかった。ntfy 経路はロック中に届くので、
緩和策ではなく要件そのものを満たす。★教訓として残す: 「本命の手段が塞がれた時の代替」を
探す前に、**要件が本命の手段を名指ししているか**を読む。名指ししていなければ、
妥協した代替ではなく**別の本物**が在り得る。
