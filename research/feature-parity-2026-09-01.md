...
| 8 | subagent / workflow を個別に止める | **在る**。同上(私が直に確認)「Stop one of them from the device, and Claude Code stops that task on your machine」 | absent | rc-backend/src/server.mjs の `action === "interrupt"` の分岐 | `interrupt` は Escape を1回送るだけで、単位がセッション全体。★**2026-09-04 調査で判定が変わった**(`research/subagent-stop-feasibility-2026-09-04.md`): 個別停止は**届く** —— agent panel(`/tasks`)で選択して `x`(changelog 2524 行)。条件 = 押す前に画面で**行の名前を突き合わせる**(パネルは自分で畳むので index は当てにならない、changelog 1529 行)。副産物: `claude agents --json` / `claude stop <id>` というセッション単位の口を机が使っていない |
