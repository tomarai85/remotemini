...
#
# ★★2026-08-30 の訂正(査読が私の誇張を捕まえた)。**此れは権限の境界を跨ぐ攻撃ではない。**
#   `~/.rc-backend` は `drwxr-xr-x athenas:staff`(ACL 無し)で、group も other も w を
#   持たない。細工の symlink を先置きできるのは **athenas 本人だけ**で、其の本人は
#   `api.key`(0600)を最初から読める。上の実演も athenas として走っている。
#   よって之は「攻撃者から守る」話ではなく、**自分の暴走から自分を守る**堅牢化。
#   `mktemp` 化は其れでも正しい(名前の衝突・残骸・並行走行に効く)が、
#   **security finding の衣を着せて記録していたのは私の誤り**。
#   ★以後「symlink 攻撃の2件目を見つけた」と言う時は、**所有者以外の主体を名指しできるか**
#     を先に問う事。名指せないなら其れは堅牢化であって脆弱性ではない。
