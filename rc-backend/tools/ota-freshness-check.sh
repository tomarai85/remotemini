#!/bin/bash
# ota-freshness-check.sh — 配布口が配っている版が、**配ってよいと決めた版**より古くないかを測る。
#
# ★見出しは 2026-08-31 に直した。元は「手元の署名済みビルドより古くないか」だったが、
#   中身は既に承認の記録と比べている。**署名済み plist は「最後に device 向けに焼いた物」**で、
#   配布とは無関係な build.sh の走行でも動く —— 見出しを信じて其れを比較の相手にすると、
#   ただの開発ビルドのたびに赤くなる(実際、生存検査が此の型で 1 件 偽の DEAD を出した)。
#
...
#
# 終了コード:
#   0 = 配っている版 >= 承認済みの版(栞を叩いても巻き戻らない)
#   1 = **配っている方が古い**(= 栞を叩くと巻き戻る)
#   2 = 測定不成立(ssh が失敗 / 必要な版番号を読めない)
#       ★「読めなかった」を「古くない」に丸めない。
#   3 = 配布は承認に追いついているが、**承認そのものが HEAD より古い**
#       = 出来ている物がまだ配る対象になっていない。巻き戻りではない。
#       ★開発中は殆ど常に此れなので、**3 を赤として扱う検査は常時赤になり読まれなくなる**。
set -uo pipefail
...
# --- 配布してよいと決めた版 ---------------------------------------------------
# ★**先に此れを読む**(2026-08-31)。順序に意味が在る —— 承認の記録が在るなら
#   署名済み plist は判定に一切使わないので、其の不在で「測定不成立」に落ちるのは
#   **測っていない入力を必須にしている**事になる。実際に其れが起きうる形だった:
#   `ios/build/` を掃除しただけで、巻き戻りの検査が黙る。CF-16 で 2 日 気付かなかった
#   穴と同じ型(「読めなかった」ではなく「読む必要が無い物を読めなくて黙る」)。
approved=""
if [ -f "$APPROVED_FILE" ]; then
    approved="$(num_or_empty "$(tr -d '[:space:]' < "$APPROVED_FILE")")"
fi

# --- 手元の署名済み版(承認の記録が無い時だけ要る)-----------------------------
# ★之は「**最後に device 向けに焼いた物**」であって「配ろうと決めた物」ではない。
#   書くのは `ios/tools/build.sh`(`OUT="$DERIVED/signed"`)で、配布とは無関係な
#   ただの device ビルドでも更新される —— 2026-08-31 に実測: 署名済み 106 /
#   承認 105 / 配布 105。106 は 08-30 17:53 の build.sh の産物で、配る対象ではない。
#   だから**比較の相手にしてよいのは、承認という出来事がまだ一度も起きていない時だけ**。
loc=""
if [ -z "$approved" ]; then
    if [ ! -f "$SIGNED_PLIST" ]; then
        echo "ota-freshness: 承認の記録($APPROVED_FILE)も署名済みビルド($SIGNED_PLIST)も無い = 測定不成立" >&2
        exit 2
    fi
    ...
    if [ -z "$loc" ]; then
        echo "ota-freshness: 承認の記録が無く、手元の版番号も読めない(実測=[$loc_raw])= 測定不成立" >&2
        exit 2
    fi
fi
