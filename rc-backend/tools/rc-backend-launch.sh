...
#   ★2026-08-04: その確認は**同じ検査の O が毎回やり直す**(この if の条件だけを false に
#     倒した写しを作り、倒れる番号がこの 5 つと**完全一致**する事を見る)。此処の一文は
#     もう「唯一の根拠」ではない —— コメントは走らないので、壊れた日には気付けなかった。
#   ★下の既定値が `tools/deploy-to-edith.sh` の既定値と一致している事は P が見ている。
#     ずれると「印は毎回立つのに守りは一度も効かない」のに L/M/N は緑のままになる
#     (L/M/N は env で置き場を渡すので、既定値のずれを構造的に見られない)。
if [ -f "$MARK" ]; then
