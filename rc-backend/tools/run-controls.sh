...
LOCAL_CTLS=(
    ...
    ../ios/tools/signout-notice-control.sh # ★2026-08-08(#37 / 監査 X2-6)。401 で
                                     # 鍵入力画面へ戻された時の「二度目である事」の運搬
                                     # (断りの文 + 前の URL)の負の対照(変異9本)。
                                     # 設計と各変異の狙いは DESIGN.md の 2.65。
                                     # ★据えた理由: 継ぎ目が4つ在って、**どの1つが切れても
                                     #   残り3つの検査は緑を出す**。断りを作る所 / 器に
                                     #   書く所 / 起動時に読む所 / body が描く所。
                                     #   単体で届くのは前3つまでで、最後の1つは
                                     #   単体からは永久に見えない —— S8-5 と同型。
                                     # ★M8 と M9 が主役(此の2本だけ UI class も撃つ)。
                                     ...
                                     #   順序を名指しで見る検査を別に置いてある。
                                     # 実測 2026-08-08: xcodebuild 10回(基準 + 変異9、
                                     # うち3回は UI class 込み)で **real 388 秒**
                                     # (= 6分28秒、DerivedData が温まっている時。
                                     # PASS 10 / FAIL 0 / UNMEASURED 0)。
                                     # ios の対照で2番目に高い —— 下の
                                     # list-return-refresh-control.sh の約8分に次ぐ。
                                     # 値段の出所は UI 検査3回で、それが此の対照の存在理由
                                     # そのものなので、安くする道は「画面に出る事を測らない」
                                     # しか無い。冷えて伸びる時は外さず温めてから回す。
