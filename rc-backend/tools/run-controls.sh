...
LOCAL_CTLS=(
    ...
    ../ios/tools/signout-notice-control.sh # ★2026-08-08(#37 / 監査 X2-6、#40 / 監査 X2-8)。
                                     # 鍵入力画面の負の対照(変異12本)。守っている物は
                                     # **2つ**在り、同じ file 群の上に載っている:
                                     ...
                                     # 実測 2026-08-08: xcodebuild 13回(基準 + 変異12、
                                     # うち5回は UI class 込み)で **real 522 秒**
                                     # (= 8分42秒、DerivedData が温まっている時。
                                     # PASS 13 / FAIL 0 / UNMEASURED 0)。
                                     # ★ios の対照で**一番高い**。下の
                                     # list-return-refresh-control.sh の 429 秒を抜いた ——
                                     # (b) を足す前(388 秒)は2番目だったので、この行は
                                     # #40 で書き換えている。値段の出所は UI 検査5回で、
