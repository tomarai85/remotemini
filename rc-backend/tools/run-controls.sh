...
LOCAL_CTLS=(
    ...
    tools/tunnel-observer-control.sh  # ★2026-08-27 新設と同時に登録。測る相手は
                                     # `tunnel-observer.sh` =「観測者が **『私が届かない』を
                                     # 『相手が死んだ』と言わない**か」。此の観測者は Tom が
                                     # 持ち歩く機体で走り、回線が落ちるのは**仕様**
                                     # (実測 2026-08-26 14:00-20:20 で 186 FAIL / 178 OK)。
                                     # ★偽陽性と偽陰性を**両方**測る —— 片方だけだと
                                     # 「何も鳴らさない実装」か「何でも鳴らす実装」で緑になる
    tools/digest-notify-control.sh    # ★2026-08-26 新設と同時に登録。測る相手は
                                     # `digest-notify.sh`。**片方だけの対照は必ず騙される**:
                                     # 「鳴った」だけ見る対照は何でも鳴らす実装で緑、
                                     # 「鳴らない」だけ見る対照は何も鳴らさない実装で緑。
                                     # だから鳴る筈(受信1回)と鳴らない筈(受信0回)を対で測り、
                                     # 受信は nonce 付きで数え、**検査が実際に走った事**も
                                     # 別に確かめる(沈黙が「検査していない」を装えない様に)
    tools/rollback-friday-control.sh  # ★2026-08-26 新設と同時に登録。測る相手は
                                     # `rollback-friday.sh`。**rsync の戻しは緑を偽装
                                     # しやすい** —— 「rsync が 0 で帰った」も「file が
                                     # 在る」も、何も戻していない実装で同じ様に出る。
                                     # だから3種類の観測可能な差(中身が違う / 世代にしか
                                     # 無い / **本番にしか無い = 消えるべき**)を作って
                                     # 3つとも解消される事を要求する。`--delete` を落とすと
                                     # 3つ目だけが赤くなる(実測)。偽 ssh / 偽 curl で駆動
    ...
    tools/mixed-index-control.sh      # ★2026-08-26 新設と同時に登録。測る相手は
                                     ...
                                     # 立てて測るので、本物の木を汚さない
    ../ios/tools/live-poll-check.sh      # ★2026-08-27 新設と同時に登録。電話の**受け取り**
                                     # (`PollClient`)を本物の rc-backend に当てる。読み・書き・
                                     # 割り込みは実サーバへ当てた事が在ったのに、**受け取りだけ
                                     # 一度も無かった** —— Tom が Claude の答えを見るのが此の経路。
                                     # 陰性対照2本(でたらめな鍵→401 / 居ない会話→404)を持ち、
                                     # 特に後者は「404 を unreachable に化けさせない」を測る
                                     # (化けると人を電波の確認へ走らせる)。
                                     # ★実トークンを使い ~60 秒。門から毎 commit 回す物ではない。
    ...
    test/account-nonblocking-controls.sh # ★2026-08-27。口座の読みが**事象ループを止めない**事。
                                     # 測るのは「速いか」ではなく「他を止めないか」 —— 応答時間
                                     # だけを見ると同期でも非同期でも `/api/account` 自身は
                                     # 同じくらいなので見分けが付かない。別の要求を同時に投げて
                                     # そちらが待たされるかを見る。実測: 本物 healthz=2ms /
                                     # 同期へ戻した写し healthz=363ms。陰性対照を同じ台本に持つ。
    test/prewarm-controls.sh         # ★2026-08-27。起動直後の1発目が冷えていない事を、
                                     # 実際にサーバを起こして測る。純関数として取り出せる部分が
                                     # 無い(温めは `server.listen` の callback)ので、これ以外に
                                     # この性質を測る道具が無い。**陰性対照を同じ台本に持つ**:
                                     # `RC_PREWARM=0` で起こした側が赤にならなければ、この検査は
                                     # 何も測っていない(実測: 温め有 read=0 / 無 read=40)。実測10秒弱。
    ...
    test/unmeasurable-staged-controls.sh # ★2026-09-09。「測れなかった」を「回す物が無い」と
                                     #   読まない事だけを測る。実測: `--would-select` を rc=3 で
                                     #   落とすと 11 本の門のうち **3 本しか走らず** commit は成功した
                                     #   (8 本が黙って飛ぶ)。壊れた門は「門が無い」より悪い ——
                                     #   門が在ると皆が思っている分だけ。U4 が陰性対照(終了コードを
                                     #   見ない旧版に戻すと素通しする)。費用: 1 秒未満。
    ...
    ../ios/tools/composer-surface-control.sh # ★2026-09-08。**画面を描く file を触ったら、画面を
                                     ...
                                     # 翌日 UI 検査を全部走らせたら **8 件 赤**(内 4 つは本物の欠陥)——
                                     # 停止ボタンの的が 30pt、選択待ちで停止ボタンが消える、打鍵中が無言、
                                     # 帯を畳んだ時に時刻の識別子ごと溶かす。全部 Tom が一番急いで触る所。
                                     # ★門を素通りした理由を実測した: `TapTargetUITests` /
                                     #   `InFlightUITests` / `AttachButtonUITests` は **対照 0 本**で、
                                     #   `ConversationView.swift` に紐づく 3 本のうち XCUITest を回すのは
                                     #   着地の負荷試験だけ、`inflight-sentence` は単体 class のみ。
                                     #   つまり画面を描く file を触っても、描いて測る検査は 1 本も回らなかった。
                                     # ★値段を承知で置く: 基準 4 class(44 件)+ 変異 2 本 = 実測 **約 40 分**。
                                     #   上の姉家族より一桁高い。それでも門に据えるのは、
                                     #   (a) 発火するのは composer の面を触った commit だけ(毎回ではない)、
                                     #   (b) 9/7 の 4 件は**此処以外のどの門も通り抜けた**から。
                                     #   ★毎回止まる門は外される(下の verdict-mutants の註)。此れは
                                     #     毎回ではないので、其の型には当たらない —— だが伸びたら
                                     #     class を削るのではなく **`-only-testing` の粒度を上げる**事。
    ...
    test/live-completion-log-controls.sh # ★2026-09-06。対照表 #32: 完了の遷移を使い捨ての会話で 1 件起こし、
                                     ...
                                     # `completion_logged=10` の様な似た欄名で通らない事が本体。
    ...
    test/disposable-reap-controls.sh # ★2026-08-28。置き去りの使い捨てを掃く `reap` の対照。
                                     # 掃除機の検査で一番大事なのは「掃く物を掃く」ではなく
                                     # **掃いてはいけない物を掃かない**事なので、そちらを先に測る。
                                     # 専用の tmux socket(-L)を噛ませた包みの中だけで建てて壊す
                                     # ので、本物の tmux サーバには触らない。実測 約 80 秒
                                     # (齢を跨がせる為に実際に待つ。`session_created` は偽れない)。
                                     # ★据えた理由: Codex 2026-08-28 が名指しした唯一残った穴 ——
                                     #   呼び出し側が SIGKILL で死ぬと trap が走らず孤児が残る。
                                     #   実際に同日 1 本出て、Tom の一覧に余分な行として見えていた。
                                     #   其の掃除が**間違って Tom の実セッションを畳む**と、
                                     #   直した物より酷い。変異(名前の門を外す)で赤 2 本まで実演済み。
    ...
    ../ios/tools/mutation-worktree-gate-controls.sh
                                     # ★2026-08-30。変異対照が**作業中の木**を書き換えるのを
                                     #   これ以上増やさない門の対照。CF-12(殺された掃引が
                                     #   3本の変異を木に残し、焼く3分前に気付いた)の
                                     #   **設計側**への手当て —— 既存の手当ては検知器2本で、
                                     #   残骸を生む設計そのものは手付かずだった。
                                     # ★中核は G1/G2/G4。検出器は書き直すたびに実物を
                                     #   取り零した(path が変数で渡る / 写しから戻す主犯を
                                     #   丸ごと / 関数経由の間接参照)。だから対照は
                                     #   「見つけるか」と「見つけ過ぎないか」の両方を撃つ。
    ../ios/tools/update-notice-ui-control.sh
                                     # ★2026-08-30。同じ帯の**電話側**。継ぎ目4本を名指しで撃つ
                                     #   (復号 / 運ぶ / 「後で」の規則 / **画素になる**)。
                                     # ★U4 と U7 は画を出す検査でしか物を言えない ——
                                     #   `.accessibilityElement(children: .contain)` の無い容器に
                                     #   識別子だけ付けても SwiftUI は要素を公開せず、
                                     #   錨を置いた気になって到達不能になる(2026-08-27 実測)。
                                     # ★U5 は辞書順で版を比べる変異。"99" > "105" なので、
                                     #   黙らせた版より新しい版まで黙る = 壁紙を消す仕掛けが
                                     #   警報そのものを消す形。
    ../ios/tools/conversation-landing-stress-control.sh
                                     # ★2026-08-31。「長い会話を開いた時、本当に一番下へ着くか」を
                                     #   **混んだ機械で 5 回**測る。1 回 緑では直った証拠にならない為
                                     #   (直す前でも 5 回中 3 回は緑だった)。組み立ては 1 回だけで、
                                     #   全核に `yes` を撒いてから `test-without-building` を 5 回。
                                     #   ★費用を隠さない: 実測で数分。掃引は 1 本ずつ順に回すので、
                                     #     此処が撒く負荷が他の対照の判定を汚す事は無い。
                                     #   ★終了コードの意味が使う場面で逆になる —— 直す**前**は
                                     #     rc=1(倒れる)が期待値で、倒れなければ計器が無力。
    ../ios/tools/mutation-residue-controls.sh
                                     # ★2026-08-30。殺された変異走行が木に残した変異を、
                                     #   何もビルドせずに名指しして戻す道具の対照。
                                     # ★測る中心は「残骸を見つけるか」ではなく
                                     #   **見えていない対照を『綺麗』と報告しないか**。
                                     #   初版は `REL_TARGETS=` だけを拾って 5 本中 1 本しか
                                     #   見えておらず、今日実際に残った ConversationView の
                                     #   変異を「残骸なし」と言う版だった —— 防ぐ筈の物を
                                     #   道具自身が作っていた。C3/C4/C5 が其の穴を声にする。
                                     # ★C6 は**本物の repo を汚して名指しされるか**で測る。
                                     #   砂場だけで緑でも、本番の対照の綴りを拾えていなければ
                                     #   役に立たない。汚した後は必ず戻す(C6b が其れを測る)。
    ../ios/tools/xcode-tree-lock-controls.sh
                                     # ★2026-08-30 登録(**新設ではない** —— 在ったのに
                                     #   一覧に無く、全掃きの「走らせる物が無い道具」に
                                     #   未登録として出ていた)。測る相手は `ios/tools/xcode-tree-lock.sh` =
                                     #   ios/ の木を同時に2本の build が奪い合わない為の錠。
                                     # ★登録の切っ掛けが此の錠の**不在**そのものだった:
                                     #   同日、全掃きの裏で私が生の `xcodebuild test` を
                                     #   同じ `-derivedDataPath build` と同じ simulator へ当てた為、
                                     #   `ios/tools/ui-fixture-behavior-control.sh` が掃引の中でだけ赤くなった。
                                     #   単独で回すと緑(rc=0)。**赤の原因は対象ではなく手順**で、
                                     #   錠を取らない外部の build を此の repo は防げない。
                                     #   だからせめて、錠そのものが壊れた時は赤くなる様にしておく。
    ../.harness/push-readiness-controls.sh
                                     # ★2026-08-30。「初めて private GitHub へ push したら
                                     #   通るか」を測る監査(CF-5 の判断材料)の対照。
                                     # ★本物の repo だけで回すと**大きさの腕が一度も
                                     #   赤くなった事の無いまま**「効いている」と読める ——
                                     #   本物は今 PII の腕で NOT READY なので、上限超の blob が
                                     #   在った時に本当に止まるかは測れていない。砂場の git repo に
                                     #   台本を写し、腕ごとに発火させる(C2 上限超 / C3 警告は止めない /
                                     #   C8 作業木から消えた blob も history に在れば見つける)。
                                     # ★C7 は「読み取り専用」を註記ではなく綴りで押さえる ——
                                     #   監査の道具が書き込みを覚えたら、それは監査ではない。
    test/app-usage-census-controls.sh
                                     # ★2026-08-30。`client=app` の行を日ごとに数えて
                                     #   上限の外へ積む道具の対照。H-3 の解除条件が
                                     #   「人が思い出して grep する」事だけに依っていて、
                                     #   同じ日に据えた上限が2週間後にその元を食い始める。
                                     # ★中核は C2a/C2b と其れぞれの変異。
                                     #   守っている機構が**2つ在る**(日が丸ごと消えた時 =
                                     #   畳み込みの引き継ぎ節 / 件数が減って見えた時 = max)。
                                     #   初版は変異を1本にまとめていて赤くならず、
    ...
    test/digest-notify-completion-log-controls.sh # ★2026-09-04。完了の遷移を「記録するだけ」の仕掛けが
                                     ...
                                     #   道具が 40 分の掃引を黙って起動していた(`ps` の親子を
                                     #   辿って発覚)。V1 が偽の掃引に印を書かせて起動の有無を
                                     #   実測し、N1 が backtick の形へ戻して V1 が空虚でない事を
                                     #   確かめる。実測1秒未満。
    ...
    test/deploy-observer-carried-controls.sh
                                     ...
                                     #   其の死角で friday の観測器が 22 日 古いまま毎日 誤報を投げた(CF-7/H-4)。
                                     #   ★env の連鎖の**連続性**まで測る —— 註記が1行入ると上半分が黙って捨てられる。
    test/observer-control-sandbox-controls.sh
                                     # ★2026-09-08。上の 2 本(parity / ota-undelivered)の変異対照が
                                     #   **殺されても実物を汚さない**事だけを測る。元は SUT が実物を指し、
                                     #   trap は砂場を消すだけで戻さなかったので、変異と復元の間で殺されると
                                     #   変異した観測器が木に残り控えも消えた —— 本番(tunnel-observer が
                                     #   source、10 分毎)が壊れた版で回る。此の suite 自身が
                                     #   `perl -e 'alarm …'` で対照を時間切れにするので SIGALRM は日常の殺し方。
                                     #   ★陰性対照つき(昔の形を合成して赤を出せる事を毎回示す)。
                                     #   費用: 実測 6 秒(対照を 5 回 起動して 0.7 秒で殺すだけ)。
    tools/ota-undelivered-observer-control.sh
                                     # ★2026-08-31。「出来ているのに配っていない」を
                                     #   配備の時でなく定期に気付く枝。撃つのが配る時と
                                     #   人の手だけだったので、配った直後は必ず緑で、
                                     #   其の後どれだけ離れても誰も言わなかった。
                                     #   ★中心は『鳴らしてはいけない時に黙るか』——
                                     #     rc=3 は作業中ほぼ常に真なので、猶予を外すと
                                     #     毎日鳴る警報になり読まれなくなる。
    ...
    test/delivery-check-controls.sh
                                     ...
                                     #   掛かっていなかった** —— 実際、其の中の時代判定は
                                     #   同じ日に 20 分で2回 壊れた。外へ出る道具に継ぎ目
                                     #   (`RC_DELIVERY_SSH` 等)を足して偽物を差せる様にし、
                                     #   机無しで「どの入力でどの判定に落ちるか」を測る。
    test/log-line-era-controls.sh
                                     # ★2026-08-31。「其のログ行は**今 走っている実装**が
                                     #   書いた物か」の判定。`delivery-check.sh` の中に在った時に
                                     #   **20 分で2回 壊れた** —— 判定そのものの書き忘れと、
                                     #   `date -j` の `-u` 落ち(UTC の刻を CDT で読み 5 時間ずれ、
                                     #   刻が未来になるので門が構造的に一度も閉じない)。
                                     #   どちらも「生きた机への ssh が要る」場所に埋まっていて
                                     #   対照が書けなかった。判定を引き剥がしたので机無しで撃てる。
    test/ota-freshness-controls.sh
                                     # ★2026-08-30。配布口が配っている版が、手元の署名済み
                                     #   ビルドより古くないかを測る検査の対照。
                                     # ★動機は実測: 配布 **89** / Tom の電話 **96** / HEAD なら **99**。
                                     #   DESIGN §11 は `~/ota` を「Tom が栞から自分で入れ直す道」と
                                     #   定めているので、**唯一の復旧経路が 7 ビルド巻き戻す**状態だった。
                                     #   2日ずれて誰も赤くならなかったのは、配った版と焼いた版を
                                     #   比べる物が木に1つも無かったから。
                                     # ★C4-C8 が主(全部「測れなかった」を 2 に落とす陰性)。
                                     #   鮮度を測れなかった時に緑を出す検査は、無い方がまし ——
                                     #   Tom が栞を叩く判断の材料になる数字だから。
    test/log-cap-live-appender-controls.sh
                                     # ★2026-08-30。上限が **launchd が抱えた追記者**の
                                     #   log を切っても書き込みを落とさないかを示す台本の
                                     #   **解析部**の対照。動機は CF-9 の未着手 ——
                                     #   上限の対照は「私が起こした追記者」相手にしか
                                     #   測っていなかった。
                                     # ★作って最初に撃ったら **406 行が消えた**(実害)。
                                     #   切る位置を退避の直後へ動かして 8-10 行まで縮めたが、
                                     #   `tail` と truncate は原子的にできないので 0 にはならない。
                                     # ★中核は D2b(境目ちょうどの抜けを掴む)。上限が切った
                                     #   瞬間に落ちるなら、抜けは必ず其の位置に出る。
                                     # ★実測そのもの(friday が要る)は
                                     #   `tools/log-cap-live-appender-proof.sh` が持つ。
    tools/parity-observer-control.sh
                                     # ★2026-08-31。配備した物が repo とずれた事を
                                     #   **配備の時でなく定期に**気付く枝の対照。
                                     #   照合2本は repo が要るので Jervis からしか回せず、
                                     #   今まで配備の前後にしか走らなかった(CF-7 の再発路)。
                                     # ★中核は「黙るべき時に黙るか」—— 此の機体は実測で
                                     #   半分の時間オフライン。H-4(毎日鳴る watchdog で
                                     #   Tom が経路ごと黙らせかけた)の再演を避ける側を先に測る。
                                     # ★P7 は「呼び出しが up の経路で到達するか」を行の前後で縛る。
                                     #   実際に `exit 0` の後ろへ置いて**一度も走らない配線**を作った。
    test/client-role-controls.sh
                                     # ★2026-08-30。私の走行と Tom の電話を数え分ける仕掛け。
                                     #   動機は H-3 訂正の実測 —— 述べ 593 件のうち 60 件が
                                     #   私の対照で、36 件だけが Tom。私は**版番号で人を
                                     #   判じて**いた(彼は普段 HEAD より古い版に留まるので、
                                     #   配る度に壊れる規則だった)。
                                     # ★中核は B1。**配布される束に control が焼かれない事**。
                                     #   焼かれれば Tom の全要求が私の対照として記録され、
                                     #   今 直している嘘の、より悪い版になる。
                                     # ★B7 は「方針でなく**成果物**を検める門」、
                                     #   B8 は「app は上限であって本人ではない」の註記を縛る
                                     #   (どちらも Codex の指摘で足した)。
    test/app-update-notice-controls.sh
                                     # ★2026-08-30。電話に「机は新しい版を配っている」と
                                     #   言わせる経路の机側。動機は CF-11 + CF-17 ——
                                     #   私が「反映済み」と報告した修正は Tom のどの版にも
                                     #   入っておらず、配布口には `client=app` が1本も来ていない。
                                     #   「新しい版が在る」を伝える経路が**私が思い出して言う**
                                     #   しか無かった。私の記憶は F3 以来この系の最弱点。
                                     # ★中核は A4(詐称できる名乗りから数字を作らない)と
                                     #   A8(既定 UA は Apple の契約であって私の物ではない)。
                                     #   後者は Codex の指摘で設計を変えた分。
    test/ota-delivery-census-controls.sh
                                     # ★2026-08-30。配布口が**実際に何本渡したか**を
                                     #   上限の外へ積む台帳の対照。動機は実測 ——
                                     #   friday の log に `client=app` の行が **path を問わず1本も無い**。
                                     #   栞は一度も叩かれていない。それを答える物が木に無かった。
                                     # ★中核は C3(切られた後も足す)と C8(置き場が掃く dir の外)。
                                     #   前者は朝の `app-usage-census` で Codex が見つけた
                                     #   「max は過大でなく**過少**」と同じ欠陥。後者を外すと
                                     #   台帳が元と一緒に消え、台帳を作る意味そのものが無くなる。
    test/observer-cap-ota-controls.sh
                                     # ★2026-08-30。「誰も見ていない機械が止まっていないか」を
                                     #   friday の観測器が言うか。DESIGN §12 に「誰も見ていない
                                     #   (既知・未着手)」と書いた穴を塞いだ回の対照。
                                     # ★中核は C14/C15/C16 —— 全部**同日の Codex 査読が見つけた実害**で、
                                     #   初版は3つとも踏んでいた。C14 = 巻き戻りを通知した後に別の
                                     #   状態を挟むと復帰通知が永久に消える(「通知済み」を現在の状態に
                                     #   括り付けていた)。C15 = --dry-run が本番の記録に「通知済み」を
                                     #   書き、一度試すと本番が黙る。C16 = 復帰通知の配達失敗を捨てて
                                     #   ok を書き、直った事を二度と言わない。
                                     # ★C2 は「試みた時刻」と「成功した時刻」を分ける事を守る ——
                                     #   毎回起動して毎回こける job は、試みだけ見ていると元気に見える。
                                     # ★C18/C19 は「測らない」と「測れない」を分ける。材料を持たない
                                     #   機体で測ろうとすれば必ず失敗し、その失敗を毎時鳴らす番人になる。
    test/fleet-plist-parity-controls.sh
                                     # ★2026-08-30。friday の com.fleet.* plist が repo と
                                     #   一致し、かつ **launchctl list に居る**かを測る検査の対照。
                                     #   本物の friday は叩かない —— 叩くと「壊れているから赤い」と
                                     #   「本番が今たまたまずれているから赤い」を読者が区別できない。
                                     #   `RC_FLEET_SSH` に偽 ssh を差して向こうの出力を作る。
                                     # ★中核は C4(中身は一致でも未 load なら赤)。**置いた事は
                                     #   動いている事ではない** —— 実例 com.edith.log-rotate-check は
                                     #   plist が在るのに launchctl list に居ない。
                                     # ★C5b は「知らない job を**消さない**と明記しているか」を測る。
                                     #   所有者不明の物を消すのがこの艦隊での最悪手なので、
                                     #   検査の**文面**まで対照の対象にしてある。
    test/log-cap-controls.sh
                                     # ★2026-08-30。log に上限を掛ける台本が、**切る過程で
                                     #   追記中の行を失わないか**を測る。「上限内に収まるか」は
                                     #   測らない —— それは行を上書きして消す初版でも通っていた。
                                     #   中核は C3(同時追記の最中に切って連番の欠けを数える)と
                                     #   C4(書き戻し方式に戻すと C3 が赤くなる事の実演)。
                                     #   ★C4 が緑に変わったら、それは「直った」ではなく
                                     #     **C3 が壊れ方を測れなくなった**合図。
                                     #   ★検出器を1度作り直している: 最初は「連番が戻った箇所」を
                                     #     数えていて C4 が緑のまま通った。上書きは順序を戻さず、
                                     #     行を消す。欠けを数える形に替えて初めて赤が出た。
                                     #   実測 14 秒(C3/C4 が書き手を回す為に各 6 秒寝る)。
)
...
# ★掃引が終わった直後に、**取りこぼし**を答えさせる(2026-08-31)。
#   上の集計は「其の回に回した物」しか知らない —— 一覧から漏れた対照は集計にも漏れるので、
#   `green=… red=… 未測定=…` は「全部を測った」の証拠にならない。門の一覧と台帳を
#   突き合わせるのは別の仕事で、其れを持っているのが `run-controls-coverage.sh`。
#   ★終了コードには混ぜない(報告であって門ではない)。掃引の三値を上書きすると、
#     「赤が在る」と「一覧から漏れている」が同じ数字に化ける。
#   ★一覧を差した走行では意味が無い(差した一覧は門の宣言と一致しなくて当然)ので飛ばす。
if [ -z "$RC_CTL_LIST_FILE" ] && [ -x "$ROOT/tools/run-controls-coverage.sh" ]; then
    echo ""
    /bin/bash "$ROOT/tools/run-controls-coverage.sh" 2>&1 | /usr/bin/sed 's/^/  /' || true
fi
