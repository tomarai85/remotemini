...
LOCAL_CTLS=(
    ...
    test/action-readback-census-controls.sh # ★2026-09-09。行為の読み戻しの台帳が、台帳
                                     #   として壊れていないか。此の repo の観測器の設計は
                                     #   「配った台本の自己申告は配った証拠ではない」に乗って
                                     #   いるのに、どの台本が読み戻しているかを**母集団として**
                                     #   数えた事が一度も無かった。A2 が要 —— 新しく足された
                                     #   変更する台本が判定を書くまで通らない事を撃つ。
                                     #   費用: 1 秒未満。
    test/path-with-space-controls.sh # ★2026-09-09。空白を含む path が来ても、対照を選ぶ
                                     #   2 つの門が黙って壊れない事。引用漏れの怖さは「落ちる」
                                     #   事ではなく **黙って別の物を見る** 事 —— `git diff -- $paths`
                                     #   に `my file.swift` が混ざると当たらない 2 つの path になり、
                                     #   git は当たらない path を無視するので其の file だけ照合から
                                     #   静かに抜ける。P2 が陰性対照(生展開では見つけられない)。
                                     #   費用: 1 秒未満。
