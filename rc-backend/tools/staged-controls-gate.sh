...
# ★2026-08-12(#66): **選んだ理由**を対照へ渡す。命令ではない ——
#   受け取った対照が之を使って自分の中を絞るかは対照が決める(使わない対照は
#   従来通り全部回る = 此処に「絞れる対照の一覧」を持たない事が要件)。
#   経緯: `ios/Sources/Core/Provisioning.swift` を1行直す commit が、其の file と
#   無関係な変異を18本回していた(実測 2026-08-12。変異20本 = 725秒の殆ど)。
#   変異は既に自分の対象 file を知っているので、**新しい宣言は作らない** ——
#   手で同期する2本目の一覧を足すのが此の repo の6回の再発の型そのもの。
#   ★渡すのは repo 相対の path。空で渡さない(空 = 手叩き = 全部回す、の意味に
#     なる規約なので、選択が在るのに空を渡すと黙って全部回る側へ倒れる)。
export RC_STAGED_FILES="$staged"
