...
# ★2026-09-03(公開写しの予行で発見): 検査 `health.test.mjs` は同じ名前を **regex のエスケープ形**
#   `edith\.tail<id>\.ts\.net` で持っていて、`.` を素の文字として要求する此の regex は其れを素通しした
#   (伏字化した全履歴に 507 箇所 残った)。`\\?` = 省略可能な backslash を各 `.` の前に許す。
#   ホスト部を省略可能にしたのは `*.tail<id>.ts.net` の様な wildcard 形も同じ理由で抜けるから。
...
if [ -n "$hits_host$hist_host$hits_hostpath$hist_hostpath" ]; then
  ...
  # ★種類3 が綺麗だった事を、**赤の時にも**言う。
  #   元は緑の枝(下の方)にしか書いていなかった。この検査は §8-3 の2件で
  #   **定常的に赤**なので、緑の枝は普段まず通らない —— つまり開示が、
  #   ほぼ実行されない側に置いてあった。結果、赤い報告の中で種類3 は
  #   「綺麗」も「見ていない」も言わず**ただ沈黙**し、読み手はそれを
  #   「問題なし」と読む。沈黙を不在と読ませるのがこのコードベースで
  #   何度も踏んでいる形なので、両方の枝で必ず一言出す。
  echo "" >&2
  if [ "$host_checked" -eq 1 ]; then
    echo "== 種類3: この機械の名前 == 一致なし(見た上で綺麗)" >&2
  else
    echo "== 種類3: この機械の名前 == ★**見ていない**(hostname が ${HOST_MIN} 文字未満)。" >&2
    echo "  この赤は機械の名前については何も言っていない。" >&2
  fi
fi
