...
# ── 観測器の木も一緒に運ぶ(2026-08-31)────────────────────────────────────
# ★此の台本は `~/rc-backend/` しか運ばない。監視器は別 dir(`~/rc-observer/`)に住み、
#   別の launchd label で走るので、**配備の守備範囲の外**に落ちていた。其の死角で
#   friday の `health-observer.sh` は repo より 139 行・22 日 古いまま動き、毎日
#   誤報を投げていた(CF-7 / H-4)。配備が観測器を置き去りに出来る形を残さない。
# ★既定は空 = **運ばない**(材料を持つ機体の殻だけが指す)。`RC_OBSERVER_DEPLOY` は
#   `deploy-to-friday.sh` の env の連鎖が渡す —— 連鎖の途中に空行も註記も入れない事
#   (2026-08-26 に2回踏んだ: `\` の次が註記だと其処で連鎖が切れ、上半分が捨てられる)。
# ★門にはしない。本体の配備は既に通っており、観測器の同期の失敗で本体を巻き戻す方が
#   害が大きい。赤は帳尻に出し、居座れば今日足した parity の見張りが定期に鳴らす。
if [ -n "${RC_OBSERVER_DEPLOY:-}" ] && [ -f "$RC_OBSERVER_DEPLOY" ]; then
    say "11. 観測器の木も追随させる(別 dir・別 label なので置き去りになりうる)"
    wl_run "観測器の追随" bash "$RC_OBSERVER_DEPLOY"
fi
