...
//   - この層は mirror root(= この常駐が走っている機体の上。台本 remote-mini.sh の
//     $MIRROR_ROOT と同じ既定)を **読む + 依頼の印を置く**だけ。worktree には触れない。
//     2026-08-27 訂正: 元は「edith 上」と書いてあったが、あの機体は 2026-08-20 に譲渡され、
//     持ち出しの既定は friday になった。常駐と持ち出し先が同じ機体に揃った(= この層が
//     読む根と、台本が書く根が一致する)事を実往復で確認済み。**この根が常駐の機体に
//     無ければ、この層は「持ち出しは無い」としか答えられない** —— 機体名を焼くのではなく、
//     常駐がどこに居るかで決まる、と読むのが正しい。
