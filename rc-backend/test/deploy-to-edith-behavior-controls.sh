...
#   → **第3弾 `test/rsync-exclude-controls.sh` が実測で塞いだ**(2026-08-03)。
#   台本から option 文字列を抜いて本物の rsync を砂場で走らせる。手元と edith の
#   両方で PASS 30/30、旗を外すと `.git/HEAD` も `.gitignore` も消える(陰性 N1/N2)。
#
