...
// ── stateOf(純関数)──────────────────────────────────────────────────────────
test("机が止めたのを見て、転写が其の後 動いていない → finished / stopped-by-desk", () => {
  const r = stateOf({ ...base(), lastActivityMs: NOW - 30_000, deskStoppedAt: NOW - 20_000 });
  assert.deepEqual(r, { state: "finished", reason: "stopped-by-desk" });
});
...
test("親が読めない / 転写が読めない時は机の観測でも unknown のまま(知らない物を知っていると言わない)", () => {
  assert.equal(stateOf({ ...base(), parentState: "unreadable", lastActivityMs: NOW - 30_000, deskStoppedAt: NOW - 20_000 }).state, "unknown");
...
test("一覧: 机が止めた行は finished / stopped-by-desk、display.state は 'Stopped'、counts.finished に数える。隣は Working のまま", () => {
  ...
  assert.equal(a1.state, "finished"); assert.equal(a1.reason, "stopped-by-desk"); assert.equal(a1.display.state, "Stopped");
