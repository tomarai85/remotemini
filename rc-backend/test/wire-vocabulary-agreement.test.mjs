...
const PHONE_ONLY = {
  ...
  // ★同じ形の5件目(一覧の名前変更、2026-09-08)。**成功する fixture では口が呼ばれたか判らない**
  //   ので、断る相へ振る為の名前空間。之が要った理由は実際に踏んだから: alert の button が
  //   `Task { … }` を積むだけなのに SwiftUI が閉じる時に対象を nil へ戻していて、本体が
  //   `guard let` で黙って return —— 押しても机の口を一度も叩かず、しかも alert は閉じるので
  //   成功と画面で見分けが付かなかった。断らせると「Can't rename」の有無が**呼ばれた事の観測**になる。
  RC_UI_RENAME_FIXTURE: "電話の環境変数名(名前変更の口が断る相へ振る = 呼ばれた事を画面から見える様にする)。線には出ない",
  ...
  //   相手にとって何秒目に見えているかを知らないし、知る必要も無い。
  //   この口が在る理由: UI 検査が本当に10秒待つと、その1本で検査全体の実行時間が跳ね上がり、
  //   やがて誰も回さなくなる(= 検査が在るのに測られない状態)。
  RC_WAIT_ESCALATE_S: "電話の環境変数名(待ちの表現を切り替える閾値を検査から縮める)。線には出ない",
  // ★2026-08-31。会話画面の着地(開いた時に最新の行まで寄る)を測る為の 4 語。
  //   線に出ないのは、**着地が電話の側だけで完結する現象**だから —— サーバは
  //   自分の返した履歴が相手の画面のどこに置かれたかを知らないし、知る必要も無い。
  //   4 語が在る理由は、此の欠陥が「倒れたか通ったか」の二値では測れないから:
  //   実物は 1 回しか捕獲できず、機械負荷 3 通り(コア飽和 / 別 simulator の並走 /
  //   主スレッド占有)すべてで再現しなかった。だから競合を待つのを止め、
  //   **故障の形を注入して**測る形にした(`ios/tools/conversation-landing-stress-control.sh`)。
  RC_UI_LANDING_SABOTAGE: "電話の環境変数名(DEBUG 限定。開いた時の錨への寄せを飛ばし『寄せが完走しなかった』形を作る)。線には出ない",
  RC_UI_LANDING_NOLOOP: "電話の環境変数名(DEBUG 限定。補正の輪を止める。輪が「働いた」でなく「必要だった」を示す陰性対照)。線には出ない",
  RC_UI_MAIN_HOG_MS: "電話の環境変数名(DEBUG 限定。app 自身の main run loop を占有する。機械負荷は QoS の差で主スレッドに届かない為)。線には出ない",
  RC_UI_MAIN_HOG_PERIOD_MS: "電話の環境変数名(DEBUG 限定。上の占有の周期)。線には出ない",
  // ★2026-08-31。通知から会話へ**着地**する経路を、機械だけで測る為の口。
  //   端から端まで(URL を投げる → 会話が開く)を 1 本で測ろうとして `simctl openurl` を
  //   使ったが、iOS が必ず「Open in "Remote Mini"?」の確認を挟むので、アプリに URL が
  //   届く前で止まる(冷えた起動でも同じ)。**外から URL を投げる経路は機械だけでは測れない。**
  //   だから鎖を測れる継ぎ目で切り、URL の解釈は単体検査、着地は此の口で id を差して画で見る。
  //   線に出ないのは、**どの会話を開くかが電話の側だけの話**だから —— サーバは
  //   相手の画面がどの会話を映しているかを知らないし、知る必要も無い。
  RC_UI_DEEPLINK: "電話の環境変数名(DEBUG 限定。起動時に開く会話の id を差し、通知からの着地を画で測る)。線には出ない",
};
...
const SERVER_ONLY = {
  ...
  PANEL: "classifyScreen の内部状態。線では `overlayOf` が UNKNOWN + overlay:\"panel\" に言い換える(status も 409 の本文も。\n    2026-09-06 の本番観測で 409 の本文だけ生の綴りが出ていたので同日に揃えた)",
  ...
  HEAD: "★語彙ではない。git の参照名(`src/gitdiff.mjs` が `git diff --shortstat HEAD` の引数に\n    書く。HEAD を付けないと staged を数えない —— 2026-09-02 Codex 批評)。線に一度も出ず、\n    電話が分岐する材料でもない。IEND と同じく、大文字の識別子を拾う走査の副産物",
  ...
  DIFF_BUSY: "diff(#4 の固め、2026-09-03)。待ち行列が一杯の時の内部の error code。応答には `reason: \"busy\"`\n    (小文字)へ言い換えてから 503 で出るので、この綴りは線に出ない(MUTEX_BUSY と同じ形)",
  DIFF_ABORTED: "同じく内部の error code。要求の側が居なくなった時で、応答は**書かない**(相手が居ない)。線に出ない",
