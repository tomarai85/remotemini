...
const SERVER_ONLY = {
  ...
  DELETE: "HTTP の動詞。サーバは受ける側で綴りを持つが、電話の `DELETE` 要求は別の書き方(下の注)",
};

/**
 * 両側に在ってよい語(= 突き合わせる対象)。此処に列挙はしない —— 集合の交わりが其れで、
 * 手で2つ目の一覧を持たない。`DELETE` だけは片側に見えるので許可表に理由が要った:
 * 電話は `URLRequest.httpMethod` へ `"DELETE"` を**書いていない**(行列の取り消しは
 * v1 の電話の画面から起こせない)。届く様になったら此処が dead 判定で赤くなる。
 */
