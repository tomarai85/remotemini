...
FX="$MS_TREE/Sources/Core/SessionsListingFixture.swift"
FR="$MS_TREE/Sources/Screens/Shared/ForegroundResume.swift"
TARGETS=("$LV" "$FX" "$FR")
...
if [ -n "$not_restored" ]; then
    ...
    echo "            = 変異ごとの復元が効いていない。砂場の中なので木への害は無いが、対照の測定は崩れる。"
    UNMEASURED=$((UNMEASURED+1))
else
    echo "復元を確認(対象 ${#TARGETS[@]} file すべて走る前のバイトと一致)"
fi

# ★走行が**本物の木を1バイトも触っていない**事を測る(Codex 2026-08-30)。
#   「触らない設計だから触っていない」は証明ではない。
