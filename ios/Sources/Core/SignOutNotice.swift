...
/// この器に鍵が入る事は無い —— `apiKey` を受け取る口すら持たないので、
/// 「入れない約束」ではなく**入れられない形**にしてある。
/// (鍵が永続する先は Keychain(`KeychainCredentialStore`)と、焼いた束の刻印
/// (`Provisioning`、2026-08-11 の種)の2箇所。此処は其のどちらでもない)
