    ...
    private func rows(_ sessions: [SessionRow], grayedOut: Bool) -> some View {
        List(sessions, id: \.id) { row in
            ...
            .contextMenu {
                ...
                //   ★場所を選ぶ画面は作らない —— 行から始めれば場所は既に決まっている。
                //   ★★2026-09-02: **此の判断の元の理由は消えた。**
                //     元は「`@` のパス補完がまだ無い以上、電話で path を打たせるのは
                //     盲打ちを強いる」だったが、補完は入った(`PathCompletionClient` /
                //     机の `src/paths.mjs`)。ただし補完が効くのは**会話の cwd の下**で、
                //     任意のディレクトリを 0 から選ぶ話(対照表 #11)は別の口が要る ——
                //     机は其の一覧を返す道を持たない。よって今は
                //     「**まだ作っていない**(#11 の仕事)」であって「作らないと決めた」ではない。
                //     前の註のまま置くと、消えた前提を根拠として次に読む人へ渡す事になる。
                Button {
                    Task {
